...
Pane_manifold_2::Pane_manifold_2(const COM::Pane       *p,
  ...
  if (parent) {
    ...
    ;  // Otherwise, they are filled by Window_manifold_2::init()
}

Pane_manifold_2::~Pane_manifold_2() {}

Halfedge Pane_manifold_2::get_prev_edge(Edge_ID eID, Access_Mode mode) const {
  Edge_ID prv;
  switch (mode) {
    case REAL_PANE:
      prv = get_prev_real_edge(eID);
      break;
    case WHOLE_PANE:
      prv = Simple_manifold_2::get_prev_edge(eID);
      break;
    case ACROSS_PANE: {
...
Halfedge Pane_manifold_2::get_next_edge(Edge_ID eID, Access_Mode mode) const {
  Edge_ID nxt;
  switch (mode) {
    case REAL_PANE:
      nxt = get_next_real_edge(eID);
      break;
    case WHOLE_PANE:
      nxt = Simple_manifold_2::get_next_edge(eID);
      break;
    case ACROSS_PANE: {
      // In the across-pane mode, it is illegal to access halfedges that are
      // not on physical boundaries but on pane boundaries. So a pane-border
...
int Pane_manifold_2::size_of_nodes(Access_Mode mode) const {
  switch (mode) {
    case REAL_PANE:
      return size_of_real_nodes();
    case WHOLE_PANE:
      return Simple_manifold_2::size_of_nodes();
    case ACROSS_PANE: {
      int n = size_of_real_nodes();
      // Loop through border nodes
      for (std::vector<Node>::const_iterator it   = _nd_prm.begin(),
                                             iend = _nd_prm.end();
           it != iend; ++it) {
...
int Pane_manifold_2::size_of_edges(Access_Mode mode) const {
  switch (mode) {
    case REAL_PANE:
      return size_of_real_edges();
    case WHOLE_PANE:
      return Simple_manifold_2::size_of_edges();
    case ACROSS_PANE: {
      int n = size_of_real_edges();

      int branch = 0;
      // Loop through border edges
      for (std::vector<Halfedge>::const_iterator it   = _bd_cnt.begin(),
                                                 iend = _bd_cnt.end();
           it != iend; ++it) {
        Halfedge hopp = *it;
        if (hopp.pane_manifold()) {
          if (hopp.pane()->id() < _pane->id())
            --n;
          else if (hopp.pane()->id() == _pane->id())
            ++branch;
        }
      }
...
void Window_manifold_2::init(const COM::Attribute *pmesh) {
  COM_assertion_msg(
      pmesh && pmesh->id() == COM::COM_MESH || pmesh->id() == COM::COM_PMESH,
  ...
  _buf_window->inherit(const_cast<COM::Attribute *>(pmesh), "", false, true,
                       NULL, 0);
  ...
  COM::Attribute *pconn_n =
      nopconn ? _buf_window->inherit(pconn_user, "", true, true, NULL, 0)
...
void Window_manifold_2::determine_counterparts(const COM::Attribute *pconn_e) {
  ...
  typedef std::map<std::pair<int, int>, std::pair<const int *, const int *>>
          B2e_map;
  B2e_map bm;
  ...
  for (int i = 0, n = _pms.size(); i < n; ++i) {
    ...
    for (int j = pconn_offset, nj = pconn_l->size_of_real_items(); j < nj;
         j += b2e_l[j + 1] + 2) {
...
void Window_manifold_2::init_communicator() {
  ...
  _cc =
      new MAP::Pane_communicator(_buf_window, _buf_window->get_communicator());
}

/// Obtain shortest edge length of incident edges of each node or element.
void Window_manifold_2::shortest_edge_length(COM::Attribute *lens) {
...
void Window_manifold_2::compute_shortest_edgelen_nodes(COM::Attribute *lens) {
  ...
  for (int i = 0, local_npanes = panes.size(); i < local_npanes; ++i, ++it) {
    ...
    for (int j = pane.size_of_elements(); j > 0; --j, ene.next()) {
      ...
      for (int k = 0, ne = ene.size_of_edges(); k < ne; k++) {
        ...
        if (nn > ne + ne)
          lp[uindex + ne + ne] = std::min(lp[uindex + ne + ne], sqlen);
...
/// Obtain shortest edge length of incident edges of each element.
void Window_manifold_2::compute_shortest_edgelen_elements(
    COM::Attribute *lens) {
  std::vector<COM::Pane *> panes;
  lens->window()->panes(panes);
  std::vector<COM::Pane *>::const_iterator it = panes.begin();
...
void Window_manifold_2::update_bd_normals(const COM::Attribute *normals,
                                          bool                  to_normalize) {
  ...
  if (normals->window() != _buf_window)
    normals_inherited =
        _buf_window->inherit(const_cast<COM::Attribute *>(normals),
                             "facenormals__CNNTEMP", false, true, NULL, 0);
  COM::Attribute *nrm =
      _buf_window->new_attribute("normals__PMTEMP", 'p', COM_DOUBLE, 3, "");
  COM::Attribute *pconn_g =
      _buf_window->new_attribute("pconn__PMTEMP", 'p', COM_INT, 1, "");
...
void Window_manifold_2::update_bd_flags(const COM::Attribute *flags) {
  ...
  if (flags->window() != _buf_window)
    flags_inherited =
        _buf_window->inherit(const_cast<COM::Attribute *>(flags),
                             "faceflags__CNNTEMP", false, true, NULL, 0);
  COM::Attribute *flg =
      _buf_window->new_attribute("flags__PMTEMP", 'p', COM_INT, 1, "");
  COM::Attribute *pconn_g =
      _buf_window->new_attribute("pconn__PMTEMP", 'p', COM_INT, 1, "");
  COM::Attribute *pconn_e = _buf_window->attribute("pconn_e");
...
void Window_manifold_2::update_bdedge_bitmap(const COM::Attribute *bitmap) {
  ...
  if (bm_inherited->window() != _buf_window)
    bm_inherited =
        _buf_window->inherit(const_cast<COM::Attribute *>(bitmap),
                             "facebm__CNNTEMP", false, true, NULL, 0);
  COM::Attribute *bm =
      _buf_window->new_attribute("bm__PMTEMP", 'p', COM_CHAR, 1, "");
  COM::Attribute *pconn_g =
      _buf_window->new_attribute("pconn__PMTEMP", 'p', COM_INT, 1, "");
  COM::Attribute *pconn_e = _buf_window->attribute("pconn_e");
  ...
  for (PM_iterator it = pm_begin(), iend = pm_end(); it != iend; ++it) {
    COM::Pane  *pn = const_cast<COM::Pane *>(it->pane());
    const char *bms_face =
        reinterpret_cast<char *>(pn->attribute(bm_inherited->id())->pointer());
...
void Window_manifold_2::accumulate_bd_values(const COM::Attribute *vals) {
  COM_assertion(vals->size_of_components() == 1);
  const COM::Attribute *vals_inherited = vals;
  // Allocate buffer arrays
  if (vals->window() != _buf_window)
    vals_inherited =
        _buf_window->inherit(const_cast<COM::Attribute *>(vals),
                             "vals__CNNTEMP", false, true, NULL, 0);
  COM::Attribute *buf =
      _buf_window->new_attribute("buf__PMTEMP", 'p', vals->data_type(), 1, "");
  COM::Attribute *pconn_g =
      _buf_window->new_attribute("pconn__PMTEMP", 'p', COM_INT, 1, "");
  COM::Attribute *pconn_e = _buf_window->attribute("pconn_e");
  ...
  for (PM_iterator it = pm_begin(), iend = pm_end(); it != iend; ++it) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      switch (vals->data_type()) {
        case COM_CHAR:
          vals_face[eid.eid() - 1] += val_bd[i];
          break;
        case COM_INT:
        case COM_INTEGER:
          (int &)vals_face[(eid.eid() - 1) * size_base_type] +=
              (int &)val_bd[i * size_base_type];
          break;
        case COM_FLOAT:
        case COM_REAL:
          (float &)vals_face[(eid.eid() - 1) * size_base_type] +=
              (float &)val_bd[i * size_base_type];
          break;
        case COM_DOUBLE:
        case COM_DOUBLE_PRECISION:
          (double &)vals_face[(eid.eid() - 1) * size_base_type] +=
              (double &)val_bd[i * size_base_type];
          break;
        default:
          COM_assertion_msg(false, "Unsupported type for reduction");
          break;
      }
    }
  }
...
void Pane_manifold_2::convert_pconn_edge2ghost(const COM::Attribute *pconn_e,
                                               COM::Attribute       *pconn_g) {
  ...
  for (int j = 1; j < n; j += b2e_l[j + 1] + 2) {
    ...
    for (int k = 0, nk = b2e_l[j + 1]; k < nk; ++k) {
      ...
      COM_assertion(buf[j + k + 3] >= 1 &&
                    buf[j + k + 3] <= size_of_real_border_edges());
    }
  }
...
void Window_manifold_2::compute_nodal_normals(COM::Attribute *nrms, int scheme,
                                              bool to_normalize,
  ...
  if (nrms->window() != _buf_window)
    nodal_normals = _buf_window->inherit(nrms, "nodal_normals__CNNTEMP", false,
                                         true, NULL, 0);
  else
    nodal_normals = nrms;

  COM::Attribute *elem_normals = _buf_window->new_attribute(
      "elem_normals__CNNTEMP", 'e', COM_DOUBLE, 3, "");
  ...
  if (to_normalize) {
    ...
    for (int i = 0, n = _cc->panes().size(); i < n; ++i, ++it) {
      ...
      for (int j = 0; j < nnodes; ++j) ptrs[j].normalize();
    }
  }
...
void Window_manifold_2::elements_to_nodes(const COM::Attribute *e_vals,
  ...
  COM_assertion_msg(
      scheme != E2N_USER || e_weights && e_weights->is_elemental(),
      "Third argument must be elemental attribute");
  COM_assertion_msg(!n_weights || n_weights->is_nodal() &&
                                      COM_compatible_types(
                                          COM_DOUBLE, n_weights->data_type()),
  ...
  if (n_vals->window() != _buf_window)
    nodal_vals = _buf_window->inherit(n_vals, "nodal_vals__E2NTEMP", false,
                                      true, NULL, 0);
  else
    nodal_vals = n_vals;
  ...
  if (e_vals->window() != _buf_window)
    elem_vals =
        _buf_window->inherit(const_cast<COM::Attribute *>(e_vals),
                             "elem_vals__E2NTEMP", false, true, NULL, 0);
  else
    elem_vals = e_vals;
  ...
  if (e_weights && e_weights->window() != _buf_window)
    elem_weights =
        _buf_window->inherit(const_cast<COM::Attribute *>(e_weights),
                             "elem_weights__E2NTEMP", false, true, NULL, 0);
  else
  ...
  for (int i = 0; i < local_npanes; ++i, ++it) {  // Loop through the panes
    ...
    const Point_3<Real> *pnts =
        reinterpret_cast<const Point_3<Real> *>(pane.coordinates());
    const COM::Attribute *elem_vals_pane = pane.attribute(elem_vals->id());
    ...
    for (int d = 1; d <= ncomp; ++d) {
      COM::Attribute *nvpi =
          ncomp == 1 ? nodal_vals_pane : (nodal_vals_pane + d);
      Real *p = reinterpret_cast<Real *>(nvpi->pointer());
      for (int j = 0, s = nvpi->stride(), n = nvpi->size_of_real_items() * s;
           j < n; j += s)
        p[j] = 0.;
    }
    ...
    for (int j = pane.size_of_real_elements(); j > 0; --j, ene.next()) {
      ...
      switch (scheme) {
        ...
        case E2N_ONE: {
          ...
          for (int k = 0; k < nn; ++k) nodal_weights_evk[k] += w;

          // Update nodal sums
          for (int d = 0; d < ncomp; ++d) {
            Real t = w * elem_vals_evk(0, d);
            for (int k = 0; k < nn; ++k) nodal_vals_evk(k, d) += t;
          }
          break;
        }
        case E2N_ANGLE:
        case E2N_SPHERE: {
          for (int k = 0; k < ne; ++k) {
            J[0]     = ps[k == ne - 1 ? 0 : k + 1] - ps[k];
            J[1]     = ps[k ? k - 1 : ne - 1] - ps[k];
            ...
            if (s > 0) {
              ...
              if (scheme == SURF::E2N_SPHERE) w = std::sin(w) / s;
  ...
  if (!tosum) {
    ...
    for (int i = 0; i < local_npanes; ++i, ++it) {  // Loop through the panes
      ...
      for (int d = 1; d <= ncomp; ++d) {
        ...
        for (int j = 0, js = nvpi->stride(),
                 n = nvpi->size_of_real_items() * js, k = 0,
                 ks = weights_strds[i];
             j < n; j += js, k += ks) {
          if (w[k] == 0) {
            std::cout << "***Rocsurf Error: Got zero weight for node " << j + 1
  ...
  if (elem_vals != e_vals) _buf_window->delete_attribute(elem_vals->name());
  if (nodal_vals != n_vals) _buf_window->delete_attribute(nodal_vals->name());
  _buf_window->init_done(false);
}

void Window_manifold_2::compute_normals(COM::Attribute *normal, int scheme,
                                        bool to_normalize) {
...
void Window_manifold_2::perturb_mesh(double range) {
  // Allocate buffer arrays
  COM::Attribute *nrm =
      _buf_window->new_attribute("normals__PMTEMP", 'n', COM_DOUBLE, 3, "");
  _buf_window->resize_array(nrm, NULL);

  COM::Attribute *rnd =
      _buf_window->new_attribute("randoms__PMTEMP", 'n', COM_DOUBLE, 1, "");
  _buf_window->resize_array(rnd, NULL);

  COM::Attribute *lens =
      _buf_window->new_attribute("lengths__PMTEMP", 'n', COM_DOUBLE, 1, "");
...
void Window_manifold_2::reduce_on_shared_nodes(COM::Attribute *attr,
                                               MPI_Op          op) {
  ...
  if (attr->window() != _buf_window) {
    ...
    attr_inherited =
        _buf_window->inherit(attr, "SuRf_ReDuCe_On_ShArEd_TeMp__",
...
int Window_manifold_2::size_of_nodes(Access_Mode mode) const {
  int n = 0;
  // Loop through panes
...
int Window_manifold_2::size_of_faces(Access_Mode mode) const {
  int n = 0;
  // Loop through panes
  PM_const_iterator it = pm_begin(), iend = pm_end();
  for (; it != iend; ++it) {
...
int Window_manifold_2::size_of_triangles(Access_Mode mode) const {
  int n = 0;
  // Loop through panes
  PM_const_iterator it = pm_begin(), iend = pm_end();
  for (; it != iend; ++it) {
...
int Window_manifold_2::size_of_quadrilaterals(Access_Mode mode) const {
  int n = 0;
  // Loop through panes
  PM_const_iterator it = pm_begin(), iend = pm_end();
  for (; it != iend; ++it) {
...
int Window_manifold_2::size_of_edges(Access_Mode mode) const {
  int n = 0;
  // Loop through panes
  PM_const_iterator it = pm_begin(), iend = pm_end();
  for (; it != iend; ++it) {
...
Vector_3<Real> get_deformed_normal(const Halfedge &h, const Vector_2<Real> &nc,
...
Vector_3<Real> Halfedge::deformed_normal(const COM::Attribute *disp) const {
...
void Window_manifold_2::assign_global_nodeIDs(
    std::vector<std::vector<int>> &gids) const {
  // Initialize the vector gids
  gids.clear();
  gids.resize(size_of_panes());
  ...
  for (int i = 0; it != iend; ++it, ++i) {
    for (int v = 0, nv = it->size_of_real_nodes(); v < nv; ++v) {
      if (it->is_primary(v + 1, mode)) gids[i][v] = ++gid;
    }
  }
...
void Window_manifold_2::serialize_window(COM::Window *outwin) const {
  ...
  for (it = pm_begin(); it != iend; ++it, ++i) {
    int fn = it->size_of_faces(REAL_PANE);
    if (fn == 0) continue;
