...
void Generic_element_2::shape_func(const Nat_coor &nc, Real N[]) const {
  switch (_nnodes) {
    case 3: {
      N[0] = 1. - nc[0] - nc[1];
      N[1] = nc[0];
...
// This function evaluates the shape function of an element. It
// computes the barycentric coordinates from the natural coordinates.
void Generic_element_2::shape_func_deriv(const Nat_coor &nc,
                                         Vector_2        Np[]) const {
  switch (_nnodes) {
    case 3: {
      Np[0][0] = -1;
    ...
    case 8: {
      ...
      Np[0][0] = eta_minus * (-3 + 4 * xi + 2 * eta);
      Np[0][1] = xi_minus * (-3 + 4 * eta + 2 * xi);
      Np[1][0] = eta_minus * (-1 + 4 * xi - 2 * eta);
      Np[1][1] = xi * (1 - 2 * xi + 4. * eta);
      Np[2][0] = eta * (-3 + 4 * xi + 2 * eta);
      Np[2][1] = xi * (-3 + 4 * eta + 2 * xi);
      Np[3][0] = eta * (1 + 4 * xi - 2 * eta);
      Np[3][1] = xi_minus * (-1 - 2 * xi + 4. * eta);
      Np[4][0] = eta_minus * (-8 * xi);
      Np[4][1] = -4 * xi * xi_minus;
      Np[5][0] = 4 * eta_minus * eta;
      Np[5][1] = xi * (-8 * eta);
      Np[6][0] = eta * (-8 * xi);
      Np[6][1] = 4 * xi * xi_minus;
      Np[7][0] = -4 * eta * eta_minus;
      Np[7][1] = xi_minus * (-8 * eta);
    }
    default:
      abort();  // Should never reach here
  }
...
Generic_element_2::Size Generic_element_2::get_num_gp(const Size doa) const {
...
Generic_element_2::Real Generic_element_2::get_gp_weight(const Size i,
...
void Generic_element_2 ::get_gp_nat_coor(const Size i, Nat_coor &nc,
                                         const Size doa) const {
  if (std::max(doa, _order) == 1) {
    if (_nedges == 3)
...
void Generic_element_2::solve(const Vector_3 J[2], Vector_3 x[],
                              const Vector_2 b[]) const {
  ...
  Vector_3 Jinv[2] = {J[0] * t[0][0] += J[1] * t[1][0],
                      J[0] * t[0][1] += J[1] * t[1][1]};
