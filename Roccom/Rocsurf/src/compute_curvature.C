...
static double compute_lbop_weights(Point_3<Real> *ps, Real *ws) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    const Vector_3<Real> u =
        (1 - eta) * (ps[1] - ps[0]) + eta * (ps[2] - ps[3]);
...
void Window_manifold_2::compute_mcn(COM::Attribute *mcn_in,
  ...
  if (mcn_in->window() != _buf_window)
    mcn = _buf_window->inherit(mcn_in, "mcn__MCNTEMP", false, true, NULL, 0);
  else
    mcn = mcn_in;

  if (lbmcn_in->window() != _buf_window)
    lbmcn =
        _buf_window->inherit(lbmcn_in, "lbmcn__MCNTEMP", false, true, NULL, 0);
  else
    lbmcn = lbmcn_in;

  // Allocate buffer spaces for weights and areas
  COM::Attribute *areas = NULL, *weights = NULL;
  areas = _buf_window->new_attribute("areas__MCNTEMP", 'n', COM_DOUBLE, 1, "");
  _buf_window->resize_array(areas, NULL);

  weights =
      _buf_window->new_attribute("weights__MCNTEMP", 'e', COM_DOUBLE, 12, "");
  ...
  for (int i = 0, local_npanes = panes.size(); i < local_npanes; ++i, ++it) {
    ...
    Vector_3<Real> *mcn_ptr =
        (Vector_3<Real> *)(pane.attribute(mcn->id())->pointer());
    Vector_3<Real> *ws_ptr =
        (Vector_3<Real> *)(pane.attribute(weights->id())->pointer());
    Real *area_ptr = (Real *)(pane.attribute(areas->id())->pointer());

    // Loop through elements of the pane
    Element_node_enumerator ene(&pane, 1);

    for (int j = 0, jsize = pane.size_of_elements(); j < jsize;
         ++j, ene.next()) {
  ...
  for (int i = 0, local_npanes = panes.size(); i < local_npanes; ++i, ++it) {
    ...
    const Vector_3<Real> *mcn_ptr =
        (const Vector_3<Real> *)(pane.attribute(mcn->id())->pointer());
    const Vector_3<Real> *ws_ptr =
        (const Vector_3<Real> *)(pane.attribute(weights->id())->pointer());

    Vector_3<Real> *lbmcn_ptr =
        (Vector_3<Real> *)(pane.attribute(lbmcn->id())->pointer());
