...
void arrange(Point_3<Real> ps_face[4], Point_3<Real> ps[8], int ind1, int ind2,
             int ind3, int ind4) {
  ps_face[0] = ps[ind1];
  ps_face[1] = ps[ind2];
  ps_face[2] = ps[ind3];
...
void arrange(Point_3<Real> ps_face[3], Point_3<Real> ps[8], int ind1, int ind2,
             int ind3) {
  ps_face[0] = ps[ind1];
  ps_face[1] = ps[ind2];
  ps_face[2] = ps[ind3];
...
void Rocsurf::compute_center(const COM::Attribute *mesh, Vector_3<Real> &cnt) {
...
void Rocsurf::compute_signed_volumes(const COM::Attribute *mesh, double *vol) {
  std::vector<const COM::Pane *> panes;
  mesh->window()->panes(panes);
