...
void project_window(const std::string& wName, const std::string& timeStr,
  ...
  for (unsigned int i = 0; i < ps.size(); ++i) {
    ...
    cout << "PANE " << i << " projecting " << pn->size_of_real_nodes()
         << " nodes." << endl;
  ...
  cerr << "RESULTS: " << pass << " of " << pn->size_of_real_nodes()
       << " points projected to themselves, average error = "
       << total_dist / total_nodes << endl;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Reading HDF file(s) " << file_in << " into window " << win_in
            << std::endl;
