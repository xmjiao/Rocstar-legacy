...
void Rocblas::calc(Attribute *z, const Attribute *x, const void *yin,
  ...
  for (zit = zpanes.begin(), zend = zpanes.end(), xit = xpanes.begin();
       zit != zend; ++zit, ++xit, yit += (ytype != BLAS_VOID && yit != NULL)) {
    ...
    const Attribute *py =
        (ytype != BLAS_VOID && yit) ? (*yit)->attribute(y->id()) : y;
...
void Rocblas::mul_scalar(const Attribute *x, const void *y, Attribute *z,
  ...
  if (att_type == COM_INT || att_type == COM_INTEGER)
    calc<std::multiplies<int>, BLAS_VOID>(z, x, y, std::multiplies<int>(),
                                          swap);
  else {
    COM_assertion_msg(
        att_type == COM_DOUBLE || att_type == COM_DOUBLE_PRECISION,
        (std::string("Unsupported data type in ") + z->fullname()).c_str());
    calc<std::multiplies<double>, BLAS_VOID>(z, x, y, std::multiplies<double>(),
                                             swap);
  }
}
...
void Rocblas::div_scalar(const Attribute *x, const void *y, Attribute *z,
  ...
  if (att_type == COM_INT || att_type == COM_INTEGER)
    ...
    COM_assertion_msg(
        att_type == COM_DOUBLE || att_type == COM_DOUBLE_PRECISION,
        (std::string("Unsupported data type in ") + z->fullname()).c_str());
    calc<std::divides<double>, BLAS_VOID>(z, x, y, std::divides<double>(),
                                          swap);
  }
}
