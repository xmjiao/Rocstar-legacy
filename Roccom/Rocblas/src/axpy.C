...
void Rocblas::axpy_gen(const void *ain, const Attribute *x, const Attribute *y,
                       Attribute *z) {
  ...
  if (atype == BLAS_VOID)
    ...
    if (!a->is_windowed()) {
      ...
      COM_assertion_msg(
          aval,
          (std::string("Caught NULL pointer in ") + a->fullname()).c_str());
    }
  }

  const int anum_dims = atype != BLAS_VOID ? a->size_of_components() : 0;
  COM_assertion_msg(
      atype == BLAS_VOID || (anum_dims == 1 || anum_dims == num_dims),
      (std::string("Numbers of components do not match between ") +
  ...
  for (zit = zpanes.begin(), zend = zpanes.end(), xit = xpanes.begin(),
    ...
    COM_assertion_msg(
        length == px->size_of_items() || xstrd == 0,
        (std::string("Numbers of items do not match between ") + x->fullname() +
    ...
    COM_assertion_msg(
        length == py->size_of_items() || ystrd == 0,
    ...
    const Attribute *pa =
        (atype != BLAS_VOID && ait) ? (*ait)->attribute(a->id()) : a;
    int        astrd = get_stride<atype>(pa);
    const bool astg  = pa && (anum_dims != num_dims || anum_dims != astrd);
    COM_assertion_msg(
        atype != BLAS_SCNE && atype != BLAS_VEC2D ||
            length == int(pa->size_of_items()) || astrd == 0,
        (std::string("Numbers of items do not match between ") + a->fullname() +
         " and " + z->fullname() + " on pane " + to_str((*zit)->id()))
            .c_str());
    ...
    if (!xstg && !zstg && !ystg && !astg && atype != BLAS_VEC &&
      ...
      for (Size i = 0, s = length * num_dims; i < s;
           ++i, ++zval, ++xval, ++yval)
      ...
      for (int i = 0; i < num_dims; ++i) {
        ...
        const Attribute *px_i =
            num_dims == 1 ? px : (*xit)->attribute(x->id() + i + 1);
        const data_type *xval = (const data_type *)px_i->pointer();
        xstrd                 = get_stride<BLAS_VEC2D>(px_i);

        const Attribute *py_i =
            num_dims == 1 ? py : (*yit)->attribute(y->id() + i + 1);
        const data_type *yval = (const data_type *)py_i->pointer();
        ystrd                 = get_stride<BLAS_VEC2D>(py_i);

        if (atype != BLAS_VOID && ait) {
          const Attribute *pa_i =
              anum_dims == 1 ? pa : (*ait)->attribute(a->id() + i + 1);
        ...
        for (int j = 0; j < length;
          ...
          *zval =
              getref<data_type, atype, 1>(aval, j, i, astrd) * *xval + *yval;
      }
    }  // end if
  }    // end for
}
