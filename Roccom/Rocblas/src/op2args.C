...
attribute, then its number of items must be 1.");
  ...
  for (zit = zpanes.begin(), zend = zpanes.end(); zit != zend;
    ...
    if (!zstg && !ystg && ytype != BLAS_VEC &&
      ...
      for (int i = 0; i < num_dims; ++i) {
        Attribute *pz_i =
            num_dims == 1 ? pz : (*zit)->attribute(z->id() + i + 1);
        result_type *zval = (result_type *)pz_i->pointer();
        zstrd             = get_stride<BLAS_VEC2D>(pz_i);

        if (ytype != BLAS_VOID && yit) {
          Attribute *py_i =
              ynum_dims == 1 ? py : (*yit)->attribute(y->id() + i + 1);
  ...
  switch (x->id()) {
    ...
    case COM::COM_ALL:
      COM_assertion_msg(false,
                        "Only the aggregate attribute atts is supported");
    default:;
  }
...
void Rocblas::rand(const Attribute *x, Attribute *z) {
  ...
  if (att_type == COM_INT || att_type == COM_INTEGER) {
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<random<int>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                          random<int>());
      else
        gen2arg<random<int>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                       random<int>());
    } else {
      if (xncomp == 1)
        gen2arg<random<int>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                        random<int>());
      else
        gen2arg<random<int>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                         random<int>());
    }
  } else {
    COM_assertion_msg(
    ...
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<random<double>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                             random<double>());
      else
        gen2arg<random<double>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                          random<double>());
    } else {
      if (xncomp == 1)
        gen2arg<random<double>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                           random<double>());
      else
        gen2arg<random<double>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                            random<double>());
    }
  }
}
...
void Rocblas::neg(const Attribute *x, Attribute *z) {
  ...
  if (att_type == COM_INT || att_type == COM_INTEGER) {
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<nega<int>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                        nega<int>());
      else
        gen2arg<nega<int>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                     nega<int>());
    } else {
      if (xncomp == 1)
        gen2arg<nega<int>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                      nega<int>());
      else
        gen2arg<nega<int>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                       nega<int>());
    }
  } else if (att_type == COM_CHAR) {
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<nega<char>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                         nega<char>());
      else
        gen2arg<nega<char>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                      nega<char>());
    } else {
      if (xncomp == 1)
        gen2arg<nega<char>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                       nega<char>());
      else
        gen2arg<nega<char>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                        nega<char>());
    }
  } else {
    COM_assertion_msg(
    ...
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<nega<double>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                           nega<double>());
      else
        gen2arg<nega<double>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                        nega<double>());
    } else {
      if (xncomp == 1)
        gen2arg<nega<double>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                         nega<double>());
      else
        gen2arg<nega<double>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                          nega<double>());
    }
  }
}
...
void Rocblas::sqrt(const Attribute *x, Attribute *z) {
  ...
  if (att_type == COM_INT || att_type == COM_INTEGER) {
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<sqrta<int>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                         sqrta<int>());
      else
        gen2arg<sqrta<int>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                      sqrta<int>());
    } else {
      if (xncomp == 1)
        gen2arg<sqrta<int>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                       sqrta<int>());
      else
        gen2arg<sqrta<int>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                        sqrta<int>());
    }
  } else {
    COM_assertion_msg(
    ...
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<sqrta<double>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                            sqrta<double>());
      else
        gen2arg<sqrta<double>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                         sqrta<double>());
    } else {
      if (xncomp == 1)
        gen2arg<sqrta<double>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                          sqrta<double>());
      else
        gen2arg<sqrta<double>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                           sqrta<double>());
    }
  }
}
...
void Rocblas::acos(const Attribute *x, Attribute *z) {
  ...
  if (att_type == COM_INT || att_type == COM_INTEGER) {
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<acosa<int>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                         acosa<int>());
      else
        gen2arg<acosa<int>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                      acosa<int>());
    } else {
      if (xncomp == 1)
        gen2arg<acosa<int>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                       acosa<int>());
      else
        gen2arg<acosa<int>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                        acosa<int>());
    }
  } else {
    COM_assertion_msg(
    ...
    if (x->is_windowed()) {
      if (xncomp == 1)
        gen2arg<acosa<double>, BLAS_SCALAR>(z, const_cast<Attribute *>(x),
                                            acosa<double>());
      else
        gen2arg<acosa<double>, BLAS_VEC>(z, const_cast<Attribute *>(x),
                                         acosa<double>());
    } else {
      if (xncomp == 1)
        gen2arg<acosa<double>, BLAS_SCNE>(z, const_cast<Attribute *>(x),
                                          acosa<double>());
      else
        gen2arg<acosa<double>, BLAS_VEC2D>(z, const_cast<Attribute *>(x),
                                           acosa<double>());
    }
  }
}
...
void Rocblas::reduce_MPI(const Attribute *x, Attribute *z, const MPI_Comm *comm,
