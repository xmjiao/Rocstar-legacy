...
void Simple_manifold_2::init(const COM::Pane         *p,
                             const Simple_manifold_2 *parent, bool with_ghost) {
  ...
  if (_is_str) {
    ...
    _with_ghost = with_ghost && (p->size_of_ghost_nodes() > 0 ||
                                 p->size_of_ghost_elements() > 0);
...
void Simple_manifold_2::determine_ghosts() {
  ...
  COM_assertion_msg(false, "Not fully implemented yet for structured mesh");
}

void Simple_manifold_2::determine_opposite_halfedges() {
  // Compute the dual connectivity.
  Pane_dual_connectivity pdc(_pane, _with_ghost);

  int nr = _nspe * _pane->size_of_real_elements();
  _oeIDs_real_or_str.clear();
  _oeIDs_real_or_str.resize(nr, Edge_ID());

  int ng = _with_ghost ? _nspe * _pane->size_of_ghost_elements() : 0;
  _oeIDs_ghost.clear();
  _oeIDs_ghost.resize(ng, Edge_ID());
  ...
  std::vector<int>        ies_common;
  ies_common.reserve(2);

  // Process real elements of unstructured meshes or structured meshes
  int nn = _is_str && _with_ghost ? _pane->size_of_elements()
  ...
  for (int j = 0; j < nn; ++j, ene.next()) {
    ...
    for (int i = 0; i < ne; ++i, ++ij) {
      ...
      ;
      std::set_intersection(ies1.begin(), ies1.end(), ies2.begin(), ies2.end(),
                            ii);
      ...
      if (eid) {
        ...
        for (int k = 0; k < nk; ++k)
          if (ene_opp[k] == node_id) {
            Edge_ID opp = Edge_ID(eid, k ? k - 1 : nk - 1);

            bool isreal_edge =
                !_with_ghost || !_is_str || is_real_element(ene.id());
...
void Simple_manifold_2::determine_incident_halfedges() {
  COM_assertion_msg(
      _is_str || int(_pane->size_of_real_nodes()) == _maxsize_real_nodes &&
  ...
  _ieIDs_real_or_str.clear();
  _ieIDs_real_or_str.resize(nnodes, Edge_ID());
  ...
  _ieIDs_ghost.clear();
  _ieIDs_ghost.resize(nnodes, Edge_ID());
...
void Simple_manifold_2::get_borders(std::vector<bool>    &is_border,
                                    std::vector<bool>    &is_isolated,
                                    std::vector<Edge_ID> *b, int *ng) const {
  ...
  if (!_is_str) {
    for (int i = 0; i < nr; ++i) is_isolated[i] = is_isolated_node(i + 1);
  }

  // Second, obtain the border nodes
  is_border.resize(nnodes, false);
  if (ng && *ng) {
    for (int i = 0; i < nr; ++i) is_border[i] = is_border_node(i + 1);

    // Loop through all the nodes
    const int offset = _maxsize_real_nodes - nr + 1, nlast = nr + *ng;
    for (int i = nr; i < nlast; ++i) is_border[i] = is_border_node(offset + i);
  } else {
    for (int i = 0; i < nr; ++i) is_border[i] = is_real_border_node(i + 1);
  }
...
#define SIZE_OF_ELMTS(prefix, name, nedges)                                 \
  int Simple_manifold_2::prefix##name() const {                             \
    int                                                    n = 0;           \
    std::vector<const COM::Connectivity *>::const_iterator it =             \
        _elem_conns.begin();                                                \
    for (; it != _elem_conns.end(); ++it) {                                 \
      if ((*it)->size_of_edges_pe() == nedges) n += (*it)->prefix##items(); \
    }                                                                       \
    return n;                                                               \
  }
