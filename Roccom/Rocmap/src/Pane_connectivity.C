...
class KD_tree_3 : public CGAL::Kdtree_d<CGAL::Kdtree_interface<Point_3<Real>>> {
...
Pane_connectivity::Pane_connectivity(const COM::Attribute *mesh, MPI_Comm c)
    : _win(mesh->window()),
      _mani2(NULL),
...
Pane_connectivity::Pane_connectivity(const Simple_manifold_2 **mani2,
                                     MPI_Comm                  c)
    : _win(mani2[0]->pane()->window()),
      _mani2(mani2),
      _comm(COMMPI_Initialized() ? c : MPI_COMM_NULL) {
  _win->panes(const_cast<std::vector<const COM::Pane *> &>(_panes));
}
...
void Pane_connectivity::collect_nodes(const std::vector<std::vector<int>> &ns,
                                      std::vector<int>     &nodes,
  ...
  // Collect the nodes
  nodes.clear();
  nodes.reserve(count + 2 * ns.size());
  pnts.clear();
  pnts.reserve(count + 2 * ns.size());
  ...
  for (int i = 0, s = _panes.size(); i < s; ++i) {
    ...
    for (std::vector<int>::const_iterator it   = ns[i].begin(),
                                          iend = ns[i].end();
         it != iend; ++it) {
      Point_3 p(0, 0, 0);
      if (!for_facet) {
        ...
        int                     vdst =
            (ene[eID.lid() + 1 == ene.size_of_edges() ? 0 : eID.lid() + 1]);
...
void Pane_connectivity::collect_points(const std::vector<std::vector<int>> &ns,
                                       std::vector<Point_3> &pnts) {
  int count = 0;
  for (unsigned int i = 0; i < ns.size(); ++i) count += ns[i].size();
  pnts.clear();
  ...
  for (int i = 0, s = _panes.size(); i < s; ++i) {
    ...
    for (std::vector<int>::const_iterator it   = ns[i].begin(),
      ...
      Point_3 p(0, 0, 0);
      int     j = *it - 1;
      for (int k = 0; k < d; ++k) p[k] = *(const double *)attr->get_addr(j, k);
...
void Pane_connectivity::determine_coisolated_nodes(const COM::Pane      &pn,
                                                   std::vector<Point_3> &pnts,
                                                   const double          tol,
  ...
  for (int j = 0; j < n; ++j) {
    Point_3 q(0, 0, 0);
    for (int k = 0; k < d; ++k) q[k] = *(const double *)attr->get_addr(j, k);
...
double Pane_connectivity::get_local_boundary_nodes(std::vector<int>     &nodes,
                                                   std::vector<Point_3> &pnts,
                                                   bool for_facet) {
  ...
  if (!for_facet) {
    ...
    if (iso_local) {  // Determine all nodes incident on isolated nodes.
      ...
      if (_comm != MPI_COMM_NULL) {
        for (int i = 0; i < comm_size; ++i) iso_counts[i] *= 3;

        std::vector<int> iso_disps(comm_size, 0);
        for (int i = 1; i < comm_size; ++i)
          iso_disps[i] = iso_disps[i - 1] + iso_counts[i - 1];

        MPI_Allgatherv(&pnts[0], iso_counts[comm_rank], MPI_DOUBLE, &pnts_g[0],
                       &iso_counts[0], &iso_disps[0], MPI_DOUBLE, _comm);
      } else
        pnts_g = pnts;
...
static bool intersect_bbox(const Point_3<Real> &bmin1,
                           ...
                           const Point_3<Real> &bmax2, Real eps) {
  // check for emptiness ??
  if (bmax1.x() + eps < bmin2.x() || bmax2.x() + eps < bmin1.x()) return false;
  if (bmax1.y() + eps < bmin2.y() || bmax2.y() + eps < bmin1.y()) return false;
  if (bmax1.z() + eps < bmin2.z() || bmax2.z() + eps < bmin1.z()) return false;
  return true;
}

static bool intersect_bbox(const Point_3<Real> &bmin, const Point_3<Real> &bmax,
                           const std::vector<Point_3<Real>> &bbox, Real tol) {
  for (int i = 0, n = bbox.size(); i < n; i += 2) {
    if (intersect_bbox(bmin, bmax, bbox[i], bbox[i + 1], tol)) return true;
  }
  return false;
}
...
static void collect_coincident_nodes(const std::vector<int>           &r_nodes,
  ...
  while (count < r_nodes.size()) {
    ...
    for (int i = 0, n = r_nodes[count++]; i < n; ++i, ++count) {
      ...
      std::vector<Point_3_ref> outList;
      outList.reserve(4);
 ...
 */
double Pane_connectivity::collect_boundary_nodes(std::vector<int>     &nodes,
                                                 std::vector<Point_3> &pnts,
                                                 bool for_facet) {
  ...
  for (int i = 1; i < comm_size; ++i) {
    ...
    MPI_Request req;
    MPI_Isend(&l_nodes[0], nodes.size(), MPI_INT, to_rank, 101, _comm, &req);
    reqs.push_back(req);
    MPI_Isend(&l_pnts[0], 3 * pnts.size(), MPI_DOUBLE, to_rank, 102, _comm,
              &req);
    reqs.push_back(req);
  }
  ...
  for (int i = comm_size - 1; i >= 1; --i) {
    ...
    MPI_Irecv(&r_nodes[0], r_nodes.size(), MPI_INT, from_rank, 101, _comm,
              &l_reqs[0]);
    MPI_Irecv(&r_pnts[0], 3 * r_pnts.size(), MPI_DOUBLE, from_rank, 102, _comm,
              &l_reqs[1]);
    ...
    if (i == comm_size - 1) {
      ...
      collect_coincident_nodes(r_nodes_pre, r_pnts_pre, bbox, local_rtree, tol,
                               nodes, pnts);
    }
  ...
  if (reqs.size()) MPI_Waitall(reqs.size(), &reqs[0], &stat[0]);
 ...
 */
void Pane_connectivity::create_b2map(std::vector<std::vector<int>> &b2v,
                                     bool                           for_facet) {
  ...
  try {
    ...
    for (unsigned int count = 0; count < nodes.size();
         count += nodes[count + 1] + 2) {
      std::fill(&panes[count], &panes[count + nodes[count + 1] + 2],
                nodes[count]);
    }
    ...
    while (count < nodes.size()) {
      ...
      for (int i = 0, n = nodes[count++]; i < n; ++i, ++count) {
        ...
        ids.reserve(outList.size());
        for (std::vector<Point_3_ref>::iterator i = outList.begin();
             i != outList.end(); ++i) {
          int offset        = i->offset();
        ...
        for (id1 = ids.begin(); id1 != ids.end(); ++id1) {
          std::vector<Node_ID>::const_iterator id2 = id1;
          ++id2;

          for (; id2 != ids.end(); ++id2) {
            if (abs(id1->first) < abs(id2->first) ||
                abs(id1->first) == abs(id2->first) && id1->second < id2->second)
              b2v_map[id1->first][id2->first].push_back(
                  pair_int(id1->second, id2->second));
            else
              b2v_map[id2->first][id1->first].push_back(
  ...
  for (i = b2v_map.begin(), iend = b2v_map.end(); i != iend; ++i) {
    ...
    if (!branchcut.empty()) {
      if (_win->pane(i->first).is_structured())
        std::cerr << "\nRocmap Info: Got a branchcut in pane " << i->first
...
void Pane_connectivity::create_b2map(COM::Attribute *pconn, bool for_facet) {
  ...
  for (int i = 0, n = b2v.size(); i != n; ++i) {
    ...
    if (_pconn_offset && cap >= 1) {
      addr[0] = cpanes;
      std::copy(b2v[i].begin(), b2v[i].begin() + std::min(size - 1, cap - 1),
                addr + 1);
    } else if (cap >= 1)
...
void Pane_connectivity::size_of_cpanes(const COM::Attribute *pconn,
                                       const int *pane_id, int *npanes_total,
                                       int *npanes_ghost) {
  ...
  const int  len_real =
      ptr_pconn ? pconn_pn->size_of_real_items() - _pconn_offset : 0;
  const int len_total =
      ptr_pconn ? pconn_pn->size_of_items() - _pconn_offset : 0;
