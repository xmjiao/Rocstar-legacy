...
void Pane_boundary::determine_border_nodes(std::vector<bool>     &is_border,
...
struct Four_tuple {
  Four_tuple(int a, int b, int c, int d) : _a(a), _b(b), _c(c), _d(d) {}
  ...
  bool       operator<(const Four_tuple &x) const {
          return _a < x._a ||
                 (_a == x._a &&
            (_b < x._b ||
             _b == x._b && (_c < x._c || _c == x._c && _d < x._d)));
  }
...
static void check_face_unique(const Four_tuple              &face_corners,
  ...
  std::pair<Corners2Face_Map::iterator, bool> ans =
      c2f.insert(std::make_pair(sorted_ns, fid));
...
void Pane_boundary::determine_border_nodes_3(std::vector<bool>     &is_border,
                                             std::vector<Facet_ID> *b,
                                             int ghost_level) {
  ...
  is_border.clear();
  is_border.resize(num_nodes, false);
  ...
  if (_pane.is_structured()) {
    ...
    // bottom
    for (int j = s * (buffer) + h, j_end = s * (1 + buffer) - ni * buffer;
         j < j_end; j += ni) {
      std::fill_n(is_border.begin() + j, w, true);
    }
    // top
    for (int j = t + h - s * buffer, end = s * nk - s * buffer - ni * buffer;
         j < end; j += ni) {
      std::fill_n(is_border.begin() + j, w, true);
    }

    // left and right
    for (int j = s * (1 + buffer) + h, stop = t - s * buffer + h; j < stop;
         j += ni * nj) {
      std::fill_n(is_border.begin() + j, w, true);
      std::fill_n(is_border.begin() + j + ni * ((nj - 1) - 2 * buffer), w,
                  true);
    }

    // upper and lower
    for (int j = s * (1 + buffer) + h; j < t - s * buffer + h; j += ni * nj) {
      for (int k = ni, k_end = ni * (nj - (1 + 2 * buffer)); k < k_end;
           k += ni) {
        is_border[j + k]                           = true;
  ...
  for (int i = 1; i <= num_elmts; ++i, ene.next()) {
    ...
    for (int j = 0, nf = ene.size_of_faces(); j < nf; ++j) {
      ...
      Four_tuple ns(fne[0], fne[1], fne[2],
                    fne.size_of_edges() > 3 ? fne[3] : -1);
      count2++;
      check_face_unique(ns, Facet_ID(i, j), c2f_vec, num_external_face,
                        counter);
    }
  }
...
void Pane_boundary::determine_isolated_nodes(std::vector<bool> &is_isolated,
                                             int                ghost_level) {
  // Initialize to true
  is_isolated.clear();

  int nnodes = ghost_level ? _pane.size_of_nodes() : _pane.size_of_real_nodes();
  ...
  int                         nelems =
      ghost_level ? _pane.size_of_elements() : _pane.size_of_real_elements();
...
double Pane_boundary::min_squared_edge_len(const std::vector<Facet_ID> &be) {
  ...
  for (it = be.begin(); it != be.end(); ++it) {
    ...
    for (int j = 0; j < fne.size_of_edges(); ++j) {
      ...
      if (attr->stride() >= 3) {
        ...
        sqdiff = square(x1[0] - x2[0]) + square(x1[1] - x2[1]) +
                 square(x1[2] - x2[2]);
      } else {
...
void Pane_boundary::determine_borders(const COM::Attribute *mesh,
  ...
  for (int i = 0, n = panes.size(); i < n; ++i) {
    ...
    pb.determine_border_nodes(is_border_bitmap, is_isolated, NULL, ghost_level);
