...
void Rocmap::compute_pconn(const COM::Attribute *mesh, COM::Attribute *pconn) {
...
void Rocmap::pane_border_nodes(const COM::Attribute *mesh,
                               COM::Attribute *isborder, int *ghost_level) {
  Pane_boundary::determine_borders(mesh, isborder,
                                   ghost_level ? *ghost_level : 0);
}
...
void Rocmap::update_ghosts(COM::Attribute *att, const COM::Attribute *pconn) {
  Pane_communicator pc(att->window(), att->window()->get_communicator());
  pc.init(att, pconn);
...
void Rocmap::load(const std::string &mname) {
  ...
  COM_set_function((mname + ".update_ghosts").c_str(), (Func_ptr)update_ghosts,
                   "iI", types);

  types[0] = types[1] = COM_METADATA;
  COM_set_function((mname + ".compute_pconn").c_str(), (Func_ptr)compute_pconn,
                   "io", types);
...
extern "C" void Rocmap_load_module(const char *mname) { Rocmap::load(mname); }
