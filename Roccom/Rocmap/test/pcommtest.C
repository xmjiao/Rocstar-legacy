...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    std::cout << "Usage: " << argv[0] << " <out_hdf_file> " << endl;
    exit(-1);
  }
  ...
  COM_call_function(IN_read, "test/surf000[01].hdf", wname.c_str());
  ...
  COM_call_function(OUT_write, argv[1], &all_hdl, (char *)wname.c_str(),
                    "0000");
