...
int main(int argc, char *argv[]) {
  ...
  if (myrank == 0) cout << "Reading mesh file \"" << argv[1] << '"' << endl;

  std::string wname("surf");
  if (myrank == 0) cout << "Creating window \"" << wname << '"' << endl;

  // Read in HDF format
  if (myrank == 0) cout << "Loading Rocin" << endl;
  ...
  if (myrank == 0) std::cout << "Resizing the attribute arrays " << endl;
  ...
  if (myrank == 0) std::cout << npanes << " panes found" << endl;

  if (myrank == 0) std::cout << "Labeling nodes with pane ids" << endl;
  ...
  if (myrank == 0) cout << "Loading Rocmap" << endl;
  ...
  if (myrank == 0) std::cout << "Updating ghost nodes." << endl;
  int MAP_update_ghost = COM_get_function_handle("MAP.update_ghosts");
  COM_call_function(MAP_update_ghost, &pid_hdl);

  if (myrank == 0) std::cout << "finishing up window initialization" << endl;
  COM_window_init_done(wname.c_str());

  if (myrank == 0) std::cout << "loading Rocout" << endl;
  ...
  COM_call_function(OUT_write, newfile.c_str(), &all_hdl, (char *)wname.c_str(),
                    "0000");
