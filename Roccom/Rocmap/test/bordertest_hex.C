...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < num_nodes; ++i) {
    cout << "Coors_s[0][" << i << "] = [" << coors_s[i][0] << " , "
         << coors_s[i][1] << " , " << coors_s[i][2] << "]" << endl;
  }
  for (int i = 0; i < num_elmts; ++i) {
    cout << "Element " << i << ": " << elmts[i][0] << " " << elmts[i][1] << " "
         << elmts[i][2] << " " << elmts[i][3] << " " << elmts[i][4] << " "
         << elmts[i][5] << " " << elmts[i][6] << " " << elmts[i][7] << endl;
  }
  ...
  COM_call_function(OUT_write, "hexmesh", &borders_hdl, "hexmesh", "000");
