...
struct Patch {
  ...
  int remoteBlockFaceNumber; /**< The remote block face number that is
                                connecting with this block */
...
inline void readTopFile(const std::string file) {
  ...
  for (int i = 0; i < HDF2PLT::StructuredGrid.numBlocks; ++i) readBlock(ifs);
...
void PrintStructured(const TT* pData, const int ndims, const int* dims_nodes,
  ...
  const int last[3] = {dims[0] - ghost, ndims >= 2 ? dims[1] - ghost : 0,
...
template <typename TT>
void PrintUnstructured(const TT* pData, int size, bool panel,
                       std::ostream& out) {
  int i;
  for (i = 0; i < size; ++i) {
    out << "  " << std::scientific
        << (!pData ? (TT)-987654321 : (panel ? pData[0] : pData[i]));
...
void COM_print_window(const std::string& wName, const std::string& timeStr,
  ...
  while (p != attrs.end()) {
    ...
    if ((*p).m_location == 'p') {
      ...
      for (i = 0; i < nPanes && okay; ++i) {
        COM_get_size((wName + '.' + (*p).m_name).c_str(), paneIds[i], &nItems,
                     &nGItems);
        if (nItems - nGItems != 1) okay = false;
      }
      if (!okay) {
        p = attrs.erase(p);
        continue;
    ...
    if ((*p).m_numComp == 1) {
      out << ", \"" << (*p).m_name << '"';
      if ((*p).m_location != 'n') elemCentered.push_back(var);
      ++var;
    } else {
      int x;
  ...
  for (i = 0; i < nPanes; ++i) {
    ...
    out << "ZONE T=\"" << std::setw(5) << std::setfill('0') << paneIds[i]
        << "\", ";
    ...
    if (nConn == 1 && std::strncmp(connNames, ":st", 3) == 0) {
      ...
      //
      //      std::cout << "Here is the pane connectivity size: " << pconnsize
      //      << std::endl;
      //      // Obtain a reference to the pconn array.
      //      const int* pconnArray = NULL;
      //      COM_get_array_const( pConnName.c_str( ), paneIds[ i ], &pconnArray
      ...
      if (useTopFile) {
        ...
        int blockNum =
            getBlockNumber(paneIds[i], HDF2PLT::StructuredGrid.numBlocks);
        std::cout << "Here is the block number: " << blockNum << std::endl;
        assert(HDF2PLT::StructuredGrid.blocks.find(blockNum) !=
               HDF2PLT::StructuredGrid.blocks.end());
        HDF2PLT::Block b = HDF2PLT::StructuredGrid.blocks[blockNum];
        std::cout << dims[0]
                  << " == " << HDF2PLT::StructuredGrid.blocks[blockNum].Ni + 7
                  << std::endl;
        std::cout << dims[1]
                  << " == " << HDF2PLT::StructuredGrid.blocks[blockNum].Nj + 7
                  << std::endl;
        std::cout << dims[2]
                  << " == " << HDF2PLT::StructuredGrid.blocks[blockNum].Nk + 7
                  << std::endl;
      }
      ...
      for (c = connInfo.begin(); c != connInfo.end(); ++c) {
        ...
        if (x != std::string::npos) (*c).m_type.erase(x);
        ...
        if (elemType == '\0' || elemType == 't' ||
            ((elemType == 'q' || elemType == 'T') && (*c).m_type[0] > 'A' &&
             (*c).m_type[0] < 'Z') ||
      ...
      if ((*p).m_numComp == 1) {
        ...
        if (dims != NULL) {  // Structured
          ...
          SwitchOnDataType(
              ...
              PrintUnstructured(
                  (TT*)pArray,
                  ((*p).m_location != 'n' ? eTotal : nNodes - ghost),
                  (*p).m_location == 'p', out));
        }
...
struct globarg {
  ...
  bool withGhost;   ///< A flag that specifies that ghost nodes need to be
                    ///< included.
  bool readControlFile;  ///< A flag that specifies that a control file needs to
                         ///< be read.
  std::string cntrlfile;   ///< Filename of the control file.
  std::string topfile;     ///< Filename for the top file.
  std::string fileregx;    ///< Regular expression that describes the files that
                           ///< need to be read.
...
void showUsage() {
  ...
  std::cout << "-top {topfile} : Specifies a top file to use to resolve the "
               "connectivity\n";
  std::cout << "-g : Enables ghost node inclusion in the output file\n";
  std::cout << "-c {cntrlfile} : Specifies a control file to use\n";
  std::cout << "-o {outputfile} : Specifies output file. Output is printed to "
               "STDOUT by default.\n";
...
int main(int argc, char* argv[]) {
  ...
  if (Program.useTopFile) readTopFile(Program.topfile);
  std::cout << "[DONE]\n";
  ...
  if (Program.outputfile == "") {
    std::cout << "Writing window out to standard output "
              << "...";
    COM_print_window(win_in, timeStr, std::cout, Program.withGhost,
                     Program.useTopFile);
    std::cout << "[DONE]\n";
  } else {
    std::cout << "Writing window out to " << Program.outputfile << "...";
    std::ofstream fout(Program.outputfile.c_str());
    COM_print_window(win_in, timeStr, fout, Program.withGhost,
                     Program.useTopFile);
    std::cout << "[DONE]\n";
  }
