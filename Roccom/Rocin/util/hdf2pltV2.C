...
struct {
  ...
  std::vector<Mesh::UnstructuredMesh>
      meshes; /*!< @brief The list of grids to write
               */
  std::vector<std::vector<bool>>
      mask; /*!< @brief Masks the points that are shared (true) share (false)
               not shared */
  std::vector<SolnMetaData> smdv; /*!< @brief The solution Metadata
                                   */
  std::vector<std::vector<std::vector<double>>>
      solution; /*!< @brief The solution data
                 */
  std::vector<PaneConnectivity>
      pconns; /*!< @brief Inter-zone (pane) connectivity information
               */
  std::map<int, int> paneIds2index; /*!< @brief Maps the pane ids to the index
                                       in the corresponding vectors */

  std::vector<std::vector<double>>
                    globalSolution; /*!< @brief Contains the global solution */
  std::vector<bool> solutionignore; /*!< @brief Masks the data to be ignored */
  std::vector<double>
      globalNodeList; /*!< @brief The global node list */
  std::vector<std::vector<int>>
      globalElementList; /*!< @brief The global element list 		 */
  ...
  /**
   * @brief Checks if the node associated with the given mesh index and point
   * index is shared.
  ...
  /**
   * @brief Returns the pane connectivity information of the mesh at the given
   * index.
...
/**
 * @brief Converts the window from block-structured format to unstructured
 * format. This function is called iff -block-structured is supplied to the
 * command line.
 * @param wname the name of the window.
 * @return newWindowName the name of the window that contains the grids in an
 * unstructured format.
 */
...
/**
 * @brief This method calls Rocmap to query the pane connectivity information
 * for the given window. A PaneConnectivity object is constructed for each mesh
 * and the shared nodes are flaged for each mesh.
 ...
 * @post Program.mask[ i ] is created for each mesh, m_i, 0 <= i <=
...
/**
 * @brief This method adds the vertex with the given x,y,z coordinates to the
 * global node list and returns the assigned global id (index).
 ...
 * @post GlobalID >= 0.
 */
int addToGlobalNodeList(const double x, const double y, const double z,
                        std::vector<double> &nodelist);

/**
 * @brief This method updates a MeshPointIdToGlobalIdMap hash with the mesh
 * panes and point IDs mapping to the global ID.
 ...
 * @pre pointId >= 0 && pointId < Program.meshes[ Program.getPaneIndex(
 * meshPaneId ) ].nc.Size( )
 * @pre globalId >= 0.
 * @pre hash != NULL.
 */
void updateHash(const int meshPaneId, const int pointId, const int globalId,
                MeshPointIdToGlobalIdMap *hash);

/**
 * @brief Writes the merged grid and its data in VTK UNSTRUCTURED_GRID file
 * format.
 * @pre Program.globalNodeList.size( ) 	  > 0.
 * @pre Program.globalElementList.size( ) > 0.
 * @pre Program.smdv.size( ) > 0.
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------
//																		F
//U N C T I O N    P R O T O T Y P E    I M P L E M E N T A T I O N
//----------------------------------------------------------------------------------------------------------------------------------------------------------
...
void writePlotFile() {
  ...
  if (!ofs.is_open()) {
    std::cerr << "Cannot write TECPLOT file: " << Program.outputfile + ".vtk"
              << std::endl;
    std::cerr << "File: " << __FILE__ << std::endl;
    std::cerr << "Line: " << __LINE__ << std::endl;
    assert(false);
  ...
  /* Write title */
  ofs << "TITLE=\"" << Program.outputfile << " " << __DATE__ << "\""
      << std::endl;
  ...
  for (int i = 0; i < Program.smdv.size(); ++i) {
    if (!Program.solutionignore[i]) {
      if (Program.smdv[i].ncomp > 1) {
        for (int component = 0; component < Program.smdv[i].ncomp;
             ++component) {
          ofs << ", \"";
  ...
  ofs << std::endl;
  ofs << "ZONE "
      << "NODES=" << Program.globalNodeList.size() / 3 << " ";
  ...
  if (!ecenteredidx.empty()) {
    ofs << "VARLOCATION=([";
    ofs << ecenteredidx[0];
    for (int i = 1; i < ecenteredidx.size(); ++i) ofs << "," << ecenteredidx[i];
    ofs << "]=CELLCENTERED)";
  }
  ofs << std::endl;
  ...
  for (int i = 0; i < Program.globalSolution.size(); ++i) {
    if (!Program.solutionignore[i]) {
      if (Program.smdv[i].ncomp > 1) {
        for (int component = 0; component < Program.smdv[i].ncomp;
             ++component) {
          ofs << "\n# Begin " << component + 1 << "-" << Program.smdv[i].name
              << std::endl;
          ...
          for (int dataidx = 0; dataidx < NumItems; ++dataidx) {
            ofs << Program.globalSolution[i][dataidx * stride + component]
                << " ";
            if (dataidx % 10 == 9) ofs << std::endl;
          }
        }
      } else {
        ofs << "\n# Begin " << Program.smdv[i].name << std::endl;
        for (int dataidx = 0; dataidx < Program.globalSolution[i].size();
             ++dataidx) {
...
void writeVtkData() {
  ...
  if (!ofs.is_open()) {
    std::cerr << "Cannot write VTK file: " << Program.outputfile + ".vtk"
              << std::endl;
    std::cerr << "File: " << __FILE__ << std::endl;
    std::cerr << "Line: " << __LINE__ << std::endl;
    assert(false);
  ...
  ofs << "CELLS " << Program.globalElementList.size() << " "
...
void printVtk(std::vector<double>           &vlist,
  ...
  for (int i = 0; i < elist.size(); ++i) {
    if (maxnodes < elist[i].size()) maxnodes = elist[i].size();
  }
  ...
  for (int i = 0; i < elist.size(); ++i) {
    ofs << elist[i].size() << " ";
    for (int j = 0; j < elist[i].size(); ++j) ofs << elist[i][j] << " ";
    ofs << std::endl;
  }
...
void updateHash(const int meshPaneId, const int pointId, const int globalId,
  ...
  assert(pointId >= 0 &&
         pointId < Program.meshes[Program.getPaneIndex(meshPaneId)].nc.Size());
  assert(globalId >= 0);
  assert(hash != NULL);
#endif
  ...
  Program.getPaneConnectivity(idx)->getPointConnectionPairs(pointId,
                                                            connections);
  ...
  for (int i = 0; i < connections.size(); ++i) {
    ...
    int remotePoint =
        Program.getPaneConnectivity(Program.getPaneIndex(remotePaneId))
            ->getSharedPointIdAt(meshPaneId, nodeIndex);
    std::pair<int, int> key = std::make_pair(remotePaneId, remotePoint);

#ifdef ASSERT_ON
    assert(remotePoint >= 0 &&
           remotePoint <
               Program.meshes[Program.getPaneIndex(remotePaneId)].nc.Size());
    assert(Program.isNodeShared(Program.getPaneIndex(remotePaneId),
                                remotePoint) == true);
#endif
 ...
 * @brief This method will stitch the partitioned solution for the single grid.
 * @param hash a hash of the local mesh points to the corresponding global Id.
 * @param ehash a hash of the lobal element ids to the corresponding global
 * element Id.
...
void stitchSolutionData(MeshPointIdToGlobalIdMap *hash,
  ...
  for (int i = 0; i < Program.globalSolution.size(); ++i) {
    if (Program.getDataItemType(i) == NODEDATA)
      Program.globalSolution[i].resize(
          Program.globalNodeList.size() * Program.smdv[i].ncomp, 0.0);
    else if (Program.getDataItemType(i) == CELLDATA)
      Program.globalSolution[i].resize(
          Program.globalElementList.size() * Program.smdv[i].ncomp, 0.0);
    else {
  ...
  for (int paneidx = 0; paneidx < Program.numberOfPanes; ++paneidx) {
    ...
    for (int dataidx = 0; dataidx < Program.solution[paneidx].size();
         ++dataidx) {
#ifdef ASSERT_ON
...
#ifdef ASSERT_ON
          assert(Program.meshes[paneidx].nc.Size() * N ==
...
#ifdef ASSERT_ON
              assert(gidx >= 0 &&
                     gidx < Program.globalSolution[dataidx].size());
              assert(lidx >= 0 &&
                     lidx < Program.solution[paneidx][dataidx].size());
#endif

              Program.globalSolution[dataidx][gidx] =
...
#ifdef ASSERT_ON
          assert(Program.meshes[paneidx].con.Nelem() * N ==
                 Program.solution[paneidx][dataidx].size());
#endif

          for (int localElement = 0; localElement < NumElements;
               ++localElement) {
#ifdef ASSERT_ON
...
void stitchGrids() {
  ...
  for (int i = 0; i < Program.meshes.size(); ++i) {
    ...
    for (int node = 0; node < mesh->nc.Size(); ++node) {
      if (!Program.isNodeShared(i, node)) {
        /* insert the node */
        int globalId =
            addToGlobalNodeList(mesh->nc.x(node + 1), mesh->nc.y(node + 1),
                                mesh->nc.z(node + 1), Program.globalNodeList);
  ...
  std::cout << "Total number of nodes: " << (Program.globalNodeList.size() / 3)
            << std::endl;
  std::cout << "Total number of elements: " << Program.globalElementList.size()
            << std::endl;
...
void maskPoints() {
  ...
  for (int pane = 0; pane < Program.numberOfPanes; ++pane) {
    ...
    /* For all remote panes, get the local shared nodes that are shared with
     * each remote pane */
    for (int rmtPane = 0; rmtPane < remotePaneIds.size(); ++rmtPane) {
      //			std::cout << "Pane: [" << Program.paneIds[ pane
      //] << "] shares "; 			std::cout << pconPtr
      //->getNumberOfNodesSharedWithRemotePane( remotePaneIds[ rmtPane ] );
      //			std::cout << " Remote pane: [" << remotePaneIds[
      //rmtPane ] << "]\n";
      ...
      assert(nodeList.size() ==
             pconPtr->getNumberOfNodesSharedWithRemotePane(id));
#endif
...
void writeVtkFiles(std::vector<Mesh::UnstructuredMesh> &meshes) {
  ...
  for (int m = 0; m < meshes.size(); ++m) {
    ...
    for (int e = 1; e <= meshes[m].con.Nelem(); ++e) {
      ofs << "8 ";
      for (int j = 1; j <= 8; ++j) ofs << meshes[m].con.Node(e, j) - 1 << " ";
      ofs << std::endl;
    }

    ofs << "CELL_TYPES " << meshes[m].con.Nelem() << std::endl;
    for (int e = 1; e <= meshes[m].con.Nelem(); ++e) ofs << "12\n";
...
std::string convertWindow(const std::string &wName) {
  ...
  TAIL_Window2UnstructuredMesh(wName, Program.meshes, Program.smdv,
                               Program.solution, 0, true);
  ...
  //		{
  //			std::cout << "\tJ: " << j << " Name: " << Program.smdv[ j ].name
  //<< " Location: " << Program.smdv[ j ].loc; 			std::cout << " Number of
  //components: " << Program.smdv[ j ].ncomp << std::endl; 			for( int k=0; k <
  //Program.solution[ i ][ j ].size( ); ++k )
  //			{
  //				std::cout << "\t\t k[" << k << "]:" << Program.solution[ i
  //][ j ][ k ] << std::endl;
  //			}
  //		}
  //	}
  ...
  for (int i = 0; i < Program.meshes.size(); ++i) {
    TAIL_UnstructuredMesh2Pane(newWindow, Program.paneIds[i], Program.meshes[i],
                               Program.smdv[i], Program.solution[i], 0);
  }
...
void printWindow(const std::string &wName) {
  ...
  if (Program.blocks) {
    ...
    // TODO: Load the Unstructured meshes from the window to the
    // data-structures.
    std::cerr << "Unstructured grids are currently not supported!\n";
    std::cerr
        << "If your data-set consists of block structured grids make sure the "
           "-block-structured argument is supplied at the command line.\n";
    std::cerr << "File: " << __FILE__ << std::endl;
    std::cerr << "Line: " << __LINE__ << std::endl;
    std::cerr << "core dumping...\n";
    assert(false);
...
std::string getWindow() {
  ...
  if (st != std::string::npos) win_in.erase(0, st + 1);
  st = win_in.find_first_not_of(
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ");
  if (st != std::string::npos) win_in.erase(st);
  ...
  if (Program.readControlFile) {
    std::cout << "Reading by control file " << Program.cntrlfile
              << " into window " << win_in << "...";
    int IN_read = COM_get_function_handle("IN.read_by_control_file");
    COM_call_function(IN_read, Program.cntrlfile.c_str(), win_in.c_str(), NULL,
                      timeStr, &len);
    std::cout << "[DONE]\n";

  } else {
    std::cout << "Reading HDF file(s) " << Program.fileregx << " into window "
              << win_in << "...";
    int IN_read = COM_get_function_handle("IN.read_window");
    COM_call_function(IN_read, Program.fileregx.c_str(), win_in.c_str(), NULL,
                      NULL, timeStr, &len);
    std::cout << "[DONE]\n";
  }
...
void showUsage() {
  ...
  std::cout << "-block-structured: Indicates the the input files are "
               "block-structured grids which must be converted to unstructured "
               "format\n";
  std::cout
      << "-o {outputfile} : Specifies output file name. Note, do not append a "
         "file extension. A file extension will be automatically appended.\n";
