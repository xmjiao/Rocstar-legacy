      ...
      for (k = ghost; k < last[2]; ++k)
        for (j = ghost; j < last[1]; ++j)
          for (i = ghost; i < last[0]; ++i) {
            for (c = 0; c < nComp; ++c)
              out << (pData[c]
                          ? pData[c][i + j * dims[0] + k * dims[0] * dims[1]]
                          : (TT)-987654321)
                  << ' ';
            out << std::endl;
          }
...
template <typename TT>
void PrintUnstructured(const TT** pData, int nComp, int size,
                       std::ostream& out) {
  int c, i;
  for (i = 0; i < size; ++i) {
    for (c = 0; c < nComp; ++c)
...
void COM_print_window(const std::string& wName, const std::string& timeStr,
  ...
  for (i = 0; i < nPanes; ++i) {
    ...
    if (nPanes == 1) {
      ...
      sout << wName << '_' << timeStr << "_" << std::setw(4)
           << std::setfill('0') << i << ".vtk";
    ...
    if (nConn == 1 && strncmp(connNames, ":st", 3) == 0) {  // Structured mesh
      ...
      for (c = connInfo.begin(); c != connInfo.end(); ++c) {
        ...
        cellTypes.push_back(std::pair<int, int>(
            COM2VTK[(*c).m_type], (*c).m_numElements - (*c).m_numGhost));
      }
    }

    out << "POINTS " << nNodes << ' ' << DataTypeAsString[attrs[0].m_dataType]
        << std::endl;
    const void* pArray[9] = {NULL, NULL, NULL, NULL, NULL,
                             NULL, NULL, NULL, NULL};
    p                     = attrs.begin();
    int comp;
    ...
    if (dims != NULL) {  // Structured
      SwitchOnDataType(
          (*p).m_dataType,
          PrintStructured((const TT**)pArray, (*p).m_numComp, ndims, dims,
                          ghost, (*p).m_location, out));
    } else {
      SwitchOnDataType(
          (*p).m_dataType,
          PrintUnstructured((const TT**)pArray, (*p).m_numComp, nNodes, out));
    }
    ...
    if (nConn != 1 || strncmp(connNames, ":st", 3) != 0) {  // Unstructured mesh
      ...
      while (t != cellTypes.end()) {
        for (i = 0; i < (*t).second; ++i) out << (*t).first << std::endl;
        ++t;
      }
    }
    ...
    if (!mesh_only && locset.count('n') > 0) {
      ...
      for (p = attrs.begin(); p != attrs.end(); ++p) {
        ...
        if ((*p).m_numComp == 1) out << "LOOKUP_TABLE default" << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  if (read_control) {
    std::cerr << "Reading by control file " << file_in << " into window "
              << win_in << std::endl;

    int IN_read = COM_get_function_handle("IN.read_by_control_file");
    COM_call_function(IN_read, file_in.c_str(), win_in.c_str(), NULL, timeStr,
                      &len);
  } else {
    std::cerr << "Reading HDF file(s) " << file_in << " into window " << win_in
              << std::endl;
