...
void COM_print_window(const std::string& wName, const std::string& timeStr,
  ...
  for (i = 0; i < nPanes; ++i) {
    ...
    if (COM_get_attribute_handle((wName + ".ridges").c_str()) > 0) {
      ...
      int n;
      COM_get_size((wName + ".ridges").c_str(), paneIds[i], &n);

      if (n > 0) {
        out << "ZONE T=\"fea" << std::setw(5) << std::setfill('0') << paneIds[i]
            << "\", N=" << nNodes - ghost << ", E=" << n
            << ", ZONETYPE=FELINESEG, DATAPACKING=BLOCK\n";
        out << "VARSHARELIST=([1-" << var << "]=" << zone_count << ")"
            << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 2 || argc > 3) {
    std::cerr << "Usage: hdf2plt [-g] <hdf input files> [<output file>]"
              << std::endl;
    std::cerr << "   or: hdf2plt [-g] -c <Rocin control file> [<output file>]"
              << std::endl;
    std::cerr
