...
void PrintConn(const int* pConn, const ConnInfo& ci, std::ostream& out) {
  ...
  for (elem = 0; elem < ne; ++elem) {
    out << etype;
    for (i = 0; i < nn; ++i) out << ' ' << (pConn[elem + i * ci.m_numElements]);
...
void COM_print_window(const std::string& wName, const std::string& timeStr,
  ...
  for (i = 0; i < nPanes; ++i) {
    ...
    if (nConn != 1 || strncmp(connNames, ":st", 3) != 0) {  // Unstructured mesh
      ...
      //       cellTypes.begin(); while (t != cellTypes.end()) {
...
int main(int argc, char* argv[]) {
  ...
  if (fail) {
    std::cerr << "Usage: cgns2smf [-meshonly] <cgns file> <smf file>"
              << std::endl;
    return 1;
  }
  ...
  std::cout << "Reading CGNS file(s) " << file_in << " into window " << win_in
            << std::endl;
