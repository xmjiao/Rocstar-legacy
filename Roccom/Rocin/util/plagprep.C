...
//=================================================================================
//				F U N C T I O N  P R O T O T Y P E  D E C L A R A T I
//O N
//=================================================================================
...
void searchTopFile() {
  ...
  if (!ifs.is_open()) {
    std::cerr << "\n Error: Cannot open input file "
              << prjname + std::string(".top") << std::endl;
    std::cerr << "File: " << __FILE__ << std::endl;
    std::cerr << "Line: " << __LINE__ << std::endl;
  }
  ...
  for (int i = 1; i <= numberOfBlocks; ++i) ofs << i << " 0 0\n";
  ...
  for (int i = 0; i < numberOfBlocks; ++i) {
    ...
    for (int j = 0; j < npatches; ++j) {
      ifs >> bc;
      std::getline(ifs, dummyline);
      if (bc == bndrycondition) ofs << blockidx << " 0\n";
    }
  }
...
void showUsage() {
  ...
  std::cout << "OPTIONS:\n";
  std::cout << "-bc {bc} sets the boundary condition used to identify "
               "injection surfaces. Default is 91.\n";
  std::cout << "-p {projectname} sets the project name, essentially the "
