...
int ChangeDirectory(const std::string &path) { return (chdir(path.c_str())); }
...
bool FILEEXISTS(const std::string &fname) {
  struct stat fstat;
  if (lstat(fname.c_str(), &fstat)) return false;
  return true;
}
...
int main(int argc, char *argv[]) {
  ...
  std::string volfiles;
  std::string junk;
  OrigSurfFile >> junk >> junk >> junk >> surffiles >> junk;
  std::getline(OrigSurfFile, surfpanes);
  OrigVolFile >> junk >> junk >> junk >> volfiles >> junk;
  ...
  for (int i = 0; i < np; i++) {
    ...
    RocinSurfFile << "@Files: " << Ostr.str() << "/" << surffiles << std::endl
                  << "@Panes: " << surfpanes << std::endl
                  << std::endl;
    RocinVolFile << "@Files: " << Ostr.str() << "/" << volfiles << std::endl
                 << "@Panes: " << volpanes << std::endl
                 << std::endl;
