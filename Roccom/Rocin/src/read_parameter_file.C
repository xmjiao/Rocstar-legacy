...
void Rocin::read_parameter_file(const char* file_name, const char* window_name,
      ...
      if (param_att) {
        ...
        switch (param_att->data_type()) {
          ...
          case COM_DOUBLE: {
            // replace 'd' or 'D' by 'e'
            char* p =
                const_cast<char*>(strpbrk(param_list[i].second.c_str(), "dD"));
            if (p) *p = 'e';
  ...
  // and populate it will all of the option value pairs
    ...
    for (uint i = 0, ni = param_list.size(); i < ni; ++i) {
      COM_new_attribute((wname + "." + param_list[i].first).c_str(), 'w',
                        COM_CHAR, 1, "");
      int param_size = param_list[i].second.size();
      COM_set_size((wname + "." + param_list[i].first).c_str(), 0,
                   param_size + 1, 0);
      void* addr;
...
void get_parameter_list(
  ...
  COM_assertion_msg(param_file, "Specified parameter file not found.");
  ...
  while (getline(param_file, cur_line)) {
    ...
    COM_assertion_msg(
        cur_pos != string::npos && cur_pos != 0,
        (std::string("Parameter file '") + file_name + "' has an invalid line.")
            .c_str());
