...
void COM_print_window(const char *wName, std::ostream &ostr) {
  ...
  for (int i = 0; i < np; ++i) {
    ...
    if (nconn == 1 && strncmp(cnames, ":st", 3) == 0) {  // Structured mesh
      ...
      ostr << "Structured mesh: " << ndims << "-D (" << nglayers
           << " ghost layers);"
           << " grid is " << dims[0];
      nnodes = dims[0];
      ...
      ostr << "Unstructured mesh: " << nnodes << " nodes (" << ngnodes
           << " ghosts)";
