...
const char *Attribute::_keywords[COM_NUM_KEYWORDS] = {
    "nc",     "1-nc",     "2-nc",     "3-nc",  "conn", "mesh", "pconn",
    "ridges", "1-ridges", "2-ridges", "pmesh", "atts", "all"};

/// The locations of keywords.
const char Attribute::_keylocs[] = {'n', 'n', 'n', 'n', 'e', 'p', 'p',
                                    'p', 'p', 'p', 'p', 'p', 'p'};

/// The datatypes of keywords.
const COM_Type Attribute::_keytypes[] = {
    COM_DOUBLE,   COM_DOUBLE,   COM_DOUBLE,  COM_DOUBLE, COM_METADATA,
    COM_METADATA, COM_INT,      COM_INT,     COM_INT,    COM_INT,
    COM_METADATA, COM_METADATA, COM_METADATA};

/// The sizes of keywords.
const COM_Type Attribute::_keysizes[] = {3, 1, 1, 1, 0, 0, 1, 2, 1, 1, 0, 0, 0};

//\}

Attribute::Attribute(Pane *pane, Attribute *parent, const std::string &name,
                     int id)
  ...
  _cap            = root->_cap;
  _strd           = root->_strd;
  _nbytes_strd    = root->_nbytes_strd;
  _unit           = root->_unit;
  _nitems         = root->_nitems;
  _ngitems        = root->_ngitems;
}
...
int Attribute::maxsize_of_items() const {
  if (_pane->ignore_ghost()) return maxsize_of_real_items();
  if (_loc == 'n' && _id != COM_NC)
...
void Attribute::set_size(int nitems, int ngitems) {
  ...
  COM_assertion((_loc != 'n' || _id == COM_NC) &&
                (_loc != 'e' || _id == COM_CONN));
  _nitems  = nitems;
  _ngitems = ngitems;
  ...
  if (_ncomp > 1) {
    ...
    for (int i = 1; i <= _ncomp; ++i) {
      this[i]._nitems  = nitems;
      this[i]._ngitems = ngitems;
    }
  }
}

void Attribute::set_pointer(void *p, int strd, int cap, int offset,
                            bool is_const) {
...
void Attribute::copy_array(void *buf, int strd, int n, int offset,
                           int direction) {
...
void Attribute::append_array(const void *from, int strd, int nitem) {
...
void *Attribute::allocate(int strd, int cap, bool force) {
  ...
  if (!force &&
      (_parent || _status == STATUS_SET || _status == STATUS_SET_CONST))
  ...
  try {
    ...
    if (nold < nnew || strd != _strd) {
      ...
      _cap         = cap;
      _strd        = strd;
      _nbytes_strd = get_sizeof(data_type(), strd);
  ...
  CATCHEXP_APPEND(Attribute::allocate)
  CATCHBADALLOC_APPEND(Attribute::allocate);
  return _ptr;
}

void Attribute::inherit(Attribute *parent, bool clone, bool withghost,
                        int depth) {
  Attribute *root = parent->root();
  _loc            = root->_loc;
  _ncomp          = root->_ncomp;
  ...
  _cap            = root->_cap;
  _strd           = root->_strd;
  _nbytes_strd    = get_sizeof(_type, _strd);
  _unit           = root->_unit;
  ...
  if (!clone) {
    ...
    if (_ncomp > 1 && _id >= 0) try {
        for (int i = 1; i <= _ncomp; ++i) {
          this[i].inherit(parent + i, clone, withghost, 1);
          if (allocated()) this[i].set_pointer(_ptr, _strd, _cap, i - 1, false);
        }
      }
    CATCHEXP_APPEND(Attribute::inherit);
  }
...
int Attribute::deallocate() {
  try {
    ...
    if (_ncomp > 1 && _id >= 0)
      for (int i = 1; i <= _ncomp; ++i) {
        this[i]._ptr    = NULL;
        this[i]._status = STATUS_NOT_INITIALIZED;
      }
  }
...
int Attribute::get_sizeof(COM_Type type, int count) {
  switch (type) {
    ...
    case COM_LONG_DOUBLE:
      return count * sizeof(long double);

    case COM_COMPLEX:
      return count * 2 * sizeof(float);
    case COM_DOUBLE_COMPLEX:
      return count * 2 * sizeof(double);

    case COM_MPI_COMMC:
      return count * sizeof(MPI_Comm);
    case COM_MPI_COMMF:
      return count * sizeof(int);
