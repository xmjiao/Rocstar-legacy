...
Connectivity *Pane::connectivity(const std::string &a, bool insert) {
  if (_id == 0) {
    throw COM_exception(
        COM_ERR_PANE_NOTEXIST,
        append_frame(_window->name() + "." + a, Pane::connectivity));
  }
  Cnct_set::iterator it = _cnct_set.begin();
  for (; it != _cnct_set.end(); ++it) {
  ...
  if (insert) {
    ...
    if (size_info) {
      Connectivity *c =
          new Connectivity(this, a, -1 - int(_cnct_set.size()), size_info);
...
Attribute *Pane::attribute(const std::string &a) {
...
Attribute *Pane::attribute(int i) {
...
Attribute *Pane::new_attribute(const std::string &aname, int aid,
...

{
  ...
  Attribute *a = new (atts) Attribute(this, aname, aid, loc, type, ncomp, unit);
  insert(a);
  ...
  if (ncomp > 1)
    for (int i = 1; i <= ncomp; ++i) {
      ...
      Attribute *ai =
          new (&atts[i]) Attribute(this, newname, aid + i, loc, type, 1, unit);
      insert(ai);
    }

  if (_id && loc == 'w') {  // Inherit window attribute
    reinterpret_cast<Attribute_friend *>(a)->inherit(_window->attribute(aid),
                                                     INHERIT_USE, true);
  }
  return a;
}
...
void Pane::insert(Attribute *attr) {
  ...
  if (_attr_set[id] == NULL)
    _attr_set[id] = attr;
  else
    COM_assertion(attr == _attr_set[id]);
}
...
void Pane::delete_attribute(int id) {
  if (id == COM_ATTS) {  // Delete all user-defined attributes.
    ...
    Attribute *a = _attr_set[id];
    COM_assertion(a);
    int ncomp = a->size_of_components();
    delete[] a;  // Delete the attribute and its components.

    // Reset its pointers in _attr_set.
    for (int i = id + ((ncomp > 1) ? ncomp : 0); i >= id; --i)
      _attr_set[i] = NULL;

    if (id + ncomp + (ncomp > 1) == int(_attr_set.size())) _attr_set.resize(id);
  }
}

void Pane::reinit_attr(int aid, OP_Init op, void **addr, int strd, int cap) {
  switch (aid) {
    case COM_CONN: {
    ...
    case COM_ATTS: {
      ...
      for (int i = COM_NUM_KEYWORDS, n = _attr_set.size(); i < n; ++i) {
        Attribute *a = _attr_set[i];
        if (a == NULL || a->pane() == NULL) continue;
        if (!a->is_windowed() || _id == 0) reinit_attr(i, op, NULL, strd, cap);
        int ncomp = a->size_of_components();
        if (ncomp > 1) i += ncomp;
      }
      return;
  ...
  if (!a->is_windowed() && !_id) {
    if (op == OP_SET || op == OP_SET_CONST)
      throw COM_exception(COM_ERR_PANE_NOTEXIST,
                          append_frame(_window->name(), Pane::reinit_attr));
    else
      return;
  }
  ...
  if (op == OP_DEALLOC) {
    ...
    if (cap == 0) {
      ...
      if (op == OP_SET || op == OP_SET_CONST)
        ...
        if (a->capacity() == 0)
          ...
          cap = a->size_of_items() +
                a->size_of_items() / 5;  // Current size pluse 20% more
      }
    }
    ...
    if (op == OP_SET || op == OP_SET_CONST) {
      ((Attribute_friend *)a)
          ->set_pointer(*addr, strd, cap, 0, op == OP_SET_CONST);
      p = *addr;
    } else {
...
void Pane::reinit_conn(Connectivity *con, OP_Init op, int **addr, int strd,
                       int cap) {
  // Assign default value for cap and strd
  if (op != OP_DEALLOC) {
    if (cap == 0) {
      // Default value for capacity is the number of items for set_array
      // but the large of the current capacity and the the number of items plus
      // some buffer.
      if (op == OP_SET || op == OP_SET_CONST)
        ...
        if (con->capacity() == 0)
          ...
          cap = con->size_of_items() +
                con->size_of_items() / 5;  // Current size pluse 20% more
      }
    }
  ...
  if (op == OP_SET || op == OP_SET_CONST) {
    ...
    ((Connectivity_friend *)con)
        ->set_pointer(*addr, strd, cap, op == OP_SET_CONST);
  } else {
...
Attribute *Pane::inherit(Attribute *from, const std::string &aname, int mode,
                         bool withghost) {
  if (from == NULL)
    throw COM_exception(
        COM_ERR_ATTRIBUTE_NOTEXIST,
        append_frame(_window->name() + "." + aname, Pane::inherit));
  ...
  switch (from->id()) {  // Process keywords
    ...
    case COM_ATTS: {
      ...
      for (int i = COM_NUM_KEYWORDS, n = atts.size(); i < n; ++i) {
        Attribute *a = atts[i];
        if (a == NULL || a->pane() == NULL) continue;
        if (!a->is_windowed() || _id == 0) inherit(a, "", mode, withghost);
        int ncomp = a->size_of_components();
        if (ncomp > 1) i += ncomp;
      }
    ...
    case COM_CONN: {
      ...
      if (mode != INHERIT_COPY) {
        ...
        if (mode == INHERIT_USE) {
          ((Attribute_friend *)_attr_set[COM_CONN])
              ->inherit(from, false, withghost);
          // Check that if structured meshes, the nodal coordinates
          // was also used.
          COM_assertion(!is_structured() ||
                        _attr_set[COM_NC]->parent() &&
                            _attr_set[COM_NC]->parent()->pane() ==
                                from->pane());
        }

        // Inherit individual connectivity tables
        std::vector<Connectivity *>::iterator it =
            from->pane()->_cnct_set.begin();
        std::vector<Connectivity *>::iterator iend =
            from->pane()->_cnct_set.end();
        for (; it != iend; ++it) {
          _cnct_set.push_back(new Connectivity(this, *it, (*it)->name(),
                                               -_cnct_set.size() - 1));
          ((Connectivity_friend *)_cnct_set.back())
              ->inherit(*it, mode, withghost);
        }
  ...
  if (mode != INHERIT_COPY) {
    ...
    if (!Attribute::compatible_types(from->data_type(), a->data_type()))
      throw COM_exception(
          ...
          append_frame(from->fullname() + " and " + a->fullname(),
                       Pane::inherit));
  }
  ...
  COM_assertion_msg(from->location() == a->location(),
                    (std::string("Location of attributes ") + from->fullname() +
                     " and " + a->fullname() + " do not match during copying.")
                        .c_str());

  // loop through components of source's data
  if (count)
    for (int j = (s_nc > 1), nj = s_nc - (s_nc == 1); j <= nj; ++j) {
      ...
      if (dest_data->pointer() && src_data->pointer()) {
        COM_assertion_msg(
            withghost &&
                    dest_data->size_of_items() == src_data->size_of_items() ||
                !withghost && dest_data->size_of_real_items() ==
                                  src_data->size_of_real_items(),
            (std::string("Number of items of attributes ") + from->fullname() +
             " and " + a->fullname() + " do not match during copying.")
                .c_str());
        try {
...
void Pane::set_size(Attribute *a, int nitems, int ng) {
  if (nitems < ng)
    throw COM_exception(COM_ERR_INVALID_SIZE,
  ...
  if (a->is_nodal()) {
    COM_assertion_msg(a->id() == COM_NC,
                      (std::string("Cannot set size for nodal attribute ") +
                       a->fullname() + ". Must use the nc attribute")
                          .c_str());
    ...
    COM_assertion_msg(a->id() == COM_CONN,
                      (std::string("Cannot set size for elemental attribute ") +
                       a->fullname() + ". Must use the conn attribute")
                          .c_str());
...
void Pane::set_size(Connectivity *con, int nitems, int ng) {
  ...
  if (!_attr_set[COM_CONN]->parent()) {
    ...
    if (nelems < 0 || ngelems < 0)
      throw COM_exception(
          ...
          append_frame(_window->name(), Pane::refresh_connectivity));
