...
void Window::set_function(const std::string &fname, Func_ptr func,
                          const std::string &intents, const COM_Type *types,
                          Attribute *a, bool is_f90) {
  if (_func_map.find(fname) != _func_map.end())
    throw COM_exception(COM_WARN_DUP_FUNC, append_frame(_name + "." + fname,
                                                        Window::init_function));

  _func_map[fname] = Function(func, intents, types, a, is_f90);
}

void Window::set_function(const std::string &fname, Member_func_ptr func,
                          const std::string &intents, const COM_Type *types,
                          Attribute *a, bool) {
  if (_func_map.find(fname) != _func_map.end())
    throw COM_exception(COM_WARN_DUP_FUNC, append_frame(_name + "." + fname,
...
Attribute *Window::new_attribute(const std::string &aname, const char loc,
                                 const int type, int ncomp,
                                 const std::string &unit) {
  // Special handing for connectivity
  if (Connectivity::is_element_name(aname) || Attribute::is_digit(aname[0]))
    throw COM_exception(
        COM_ERR_INVALID_ATTRIBUTE_NAME,
        append_frame(_name + "." + aname, Window::new_attribute));
  if (ncomp <= 0)
    throw COM_exception(
  ...
  Attribute *a =
      ((Pane_friend &)_dummy).new_attribute(aname, id, loc, type, ncomp, unit);
  _attr_map[aname] = a;
...
void Window::delete_attribute(const std::string &aname) {
  Attr_map::iterator it = _attr_map.find(aname);
  if (it == _attr_map.end())
    throw COM_exception(
        COM_ERR_ATTRIBUTE_NOTEXIST,
        append_frame(_name + "." + aname, Window::delete_attribute));
...
void Window::set_size(const std::string &aname, int pid, int nitems, int ng) {
  if (Connectivity::is_element_name(aname)) {
    ...
    if (a == NULL)
      throw COM_exception(COM_ERR_ATTRIBUTE_NOTEXIST,
                          append_frame(_name + "." + aname, Window::set_size));
    ...
    if (a->is_windowed())
      ...
      for (Pane_map::iterator it = _pane_map.begin(), iend = _pane_map.end();
           it != iend; ++it) {
        ((Pane_friend *)it->second)
            ->set_size(it->second->attribute(a->id()), nitems, ng);
      }
    } else
...
void Window::set_array(const std::string &aname, const int pane_id, void *addr,
...

{
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(name() + "." + aname, Window::set_array));
    ...
    reinit_attr(a, is_const ? Pane::OP_SET_CONST : Pane::OP_SET, &addr, strd,
                cap);
  }
}
...
void Window::alloc_array(const std::string &aname, const int pane_id,
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(name() + "." + aname, Window::alloc_array));
...
void Window::resize_array(const std::string &aname, const int pane_id,
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(name() + "." + aname, Window::resize_array));
...
void Window::append_array(const std::string &aname, const int pane_id,
  ...
  if (a == NULL)
    throw COM_exception(
        COM_ERR_ATTRIBUTE_NOTEXIST,
        append_frame(name() + "." + aname, Window::append_array));
  try {
...
void Window::dealloc_array(const std::string &aname, const int pane_id) {
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(name() + "." + aname, Window::dealloc_array));
...
Attribute *Window::inherit(Attribute *from, const std::string &aname, int mode,
                           bool withghost, const Attribute *cond, int val)

{
  ...
  if (with_conn && aname.size() && from->name() != aname)
    throw COM_exception(
        ...
        append_frame(from->fullname() + " and " + aname, Window::inherit));

  if (cond && cond->window() != from->window()) {
    throw COM_exception(
        COM_ERR_INCOMPATIBLE_ATTRS,
        append_frame(from->fullname() + " and " + cond->fullname(),
                     Window::inherit));
  }
  ...
  for (Pane_map::iterator ppit = ps.begin(); ppit != ps.end(); ++ppit) {
    ...
    if (!cond && (val == 0 || val == ppn->id()) || cnd && *cnd == val) {
      ...
      if (pn) pn->inherit(ppn->attribute(from->id()), aname, mode, withghost);
    }
  }
  return a;
}
...
void Window::init_done(bool pane_changed) {
  ...
  if (_status == STATUS_SHRUNK) {
    int max_id = 0;
    for (Attr_map::iterator it = _attr_map.begin(); it != _attr_map.end();
         ++it) {
  ...
  if (!pane_changed) {
    if (npanes > int(_pane_map.size())) {
      throw COM_exception(COM_ERR_INIT_DONE_PANEMAP,
                          append_frame(_name, Window::init_done));
    }
    return;
  }
...
Attribute *Window::get_attribute(const std::string &aname, char *loc, int *type,
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (ncomp)
      *ncomp = Connectivity::get_size_info(aname)[Connectivity::SIZE_NNODES];
    if (unit) *unit = "";
    ...
    if (it == _attr_map.end())
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(_name + "." + aname, Window::get_attribute));
...

{
  if (pid == 0 && a->location() != 'w')
    throw COM_exception(COM_ERR_NOT_A_WINDOW_ATTRIBUTE,
                        append_frame(a->fullname(), Window::get_size));

  if (nitem) *nitem = a->size_of_items();
  if (ng) *ng = a->size_of_ghost_items();
}

void Window::get_size(const std::string &aname, int pid, int *nitem,
                      int *ng) const {
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (conn == NULL)
      throw COM_exception(COM_ERR_ATTRIBUTE_NOTEXIST,
                          append_frame(name() + "." + aname, Window::get_size));
...
int Window::get_status(const std::string &aname, int pid) const {
...
void Window::get_parent(const std::string &aname, int pid,
  ...
  try {
    ...
    if (Connectivity::is_element_name(aname)) {
      ...
      if (conn == NULL)
        throw COM_exception(
            COM_ERR_ATTRIBUTE_NOTEXIST,
            append_frame(name() + "." + aname, Window::get_parent));

      conn = conn->parent();
      if (conn == NULL)
        parent.clear();
      else
        parent = conn->fullname();
    } else {
      // Get the corresponding attribute object.
      const COM::Attribute *attr = pn->attribute(aname);
      if (attr == NULL)
...
template <class Attr>
void get_array_common(const Attr *a, int pid, Window::Pointer_descriptor &addr,
                      int *strd, int *cap, bool is_const) {
  if (!is_const && a->is_const())
    throw COM_exception(COM_ERR_ATTRIBUTE_CONST,
                        append_frame(a->fullname(), Window::get_array));
  ...
  if (addr.dim == 1) {
    ...
    if (addr.dim == 0 && (a->capacity() != 1 || a->size_of_components() != 1))
      throw COM_exception(COM_ERR_INVALID_SIZE,
                          append_frame(a->fullname(), Window::get_array));
    if (addr.dim > 2)
      throw COM_exception(COM_ERR_INVALID_DIMENSION,
                          append_frame(a->fullname(), Window::get_array));
  }
...
void Window::get_array(const std::string &aname, const int pane_id,
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (con == NULL)
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(name() + "." + aname, Window::get_array));
...
inline void copy_array_common(const Attr *a, int pid, void *val, int v_strd,
  ...
  if (pid == 0 && a->location() != 'w')
    throw COM_exception(COM_ERR_NOT_A_WINDOW_ATTRIBUTE,
                        append_frame(a->fullname(), Window::copy_size));

  const_cast<Attr *>(a)->copy_array(val, v_strd, v_size, offset,
                                    Attribute::COPY_OUT);
}

void Window::copy_array(const std::string &aname, const int pane_id, void *val,
                        int v_strd, int v_size, int offset) const {
  const Pane_friend *pn;
  try {
    pn = &(Pane_friend &)pane(pane_id);
  } catch (COM_exception ex) {
  ...
  if (Connectivity::is_element_name(aname)) {
    ...
    if (conn == NULL)
      throw COM_exception(
          COM_ERR_ATTRIBUTE_NOTEXIST,
          append_frame(name() + "." + aname, Window::copy_array));

    copy_array_common(conn, pane_id, val, v_strd, v_size, offset);
  } else {
    ...
    copy_array_common(attr, pane_id, val, v_strd, v_size, offset);
  }
}

void Window::reinit_attr(Attribute *a, OP_Init op, void **addr, int strd,
                         int cap) {
  int aid = a->id();
...
void Window::reinit_conn(Connectivity *con, OP_Init op, int **addr, int strd,
                         int cap) {
  Pane *pn = con->pane();
  if (pn->id() == 0) {
    COM_assertion(op != Pane::OP_SET && op != Pane::OP_SET_CONST);
  ...
  if (pit == _pane_map.end()) {
    if (insert)
      ...
      throw COM_exception(COM_ERR_PANE_NOTEXIST,
                          append_frame(_name, Window::pane));
    }
  }
...
void Window::panes(std::vector<int> &pane_ids, int rank) {
  ...
  if (rank == -2) {
    ...
    for (it = _proc_map.begin(), iend = _proc_map.end(); it != iend; ++it) {
      if (rank == -1 || it->second == rank) pane_ids.push_back(it->first);
    }
  }
}
...
void Window::panes(std::vector<Pane *> &ps) {
  ...
  for (; it != _pane_map.end(); ++it) ps.push_back(it->second);
}

Attribute *Window::attribute(const std::string &aname) {
  if (!Attribute::is_digit(aname[0])) {
    ...
    if (start == aname.npos)
      throw COM_exception(COM_ERR_INVALID_ATTRIBUTE_NAME,
                          append_frame(_name + "." + aname, Window::attribute));
...
Function *Window::function(const std::string &fname) {
