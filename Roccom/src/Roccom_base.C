...
Roccom_base::Roccom_base(int *argc, char ***argv)
  ...
  if (!COMMPI_Initialized()) MPI_Init(argc, argv);
  //  }
  ...
  if (_debug && rank == 0) {
    std::cerr << "Roccom Version 3.0. CVS $Id: Roccom_base.C,v 1.93 2008/12/06 "
                 "08:43:25 mtcampbe Exp $"
              << std::endl;
...
void Roccom_base::load_module(const std::string &lname,
                              const std::string &wname) {
#ifndef STATIC_LINK
  if (_debug)
    std::cerr << "Roccom: Loading module " << lname << " with arguments "
              << wname << "..." << std::endl;
  ...
  if (index < 0) {  // Load the library
    ...
    if (handle == NULL) std::cerr << "dlopen error: " << dlerror() << std::endl;
    ...
    if (handle == NULL) {
      ...
      if (_libdir.empty())
        ...
        libs = lname_full + " or " + lname_short;
  ...
  if (fptr != NULL) {
    ...
    if (fptr != NULL) {
      ...
      std::string msg;
      if (ibegin == iend)
        msg = fname + " in " + lname_found;
      else
        msg = fname + " or any its lowercase w/o underscore in " + lname;
...
void Roccom_base::unload_module(const std::string &lname,
  ...
  std::set<std::string>::iterator it =
      wname.empty() ? obj.begin() : obj.find(wname);
  ...
  if (fptr != NULL) {
    ...
    if (fptr != NULL) {
      ...
      proc_exception(
          COM_exception(COM_ERR_COULD_FINDSYM, fname + " in " + lname),
          "Roccom_base::unload_module");
    }
  }
...
void Roccom_base::new_window(const std::string &name, MPI_Comm comm) {
  ...
  try {
    ...
    std::string s("When processing ");
    s.append(name);
    proc_exception(ex, s);
  }
}

void Roccom_base::window_init_done(const std::string &wname, bool panechanged) {
  try {
    ...
    std::string msg =
        std::string("When completing the init of window ") + wname;
    proc_exception(ex, msg);
  }
  if (_debug)
    std::cerr << "Roccom: Done creating window \"" << wname << '"' << std::endl;
}
...
void Roccom_base::delete_pane(const std::string &wname, const int pane_id) {
  try {
    if (_debug)
    ...
    std::string msg =
        std::string("When processing window ") + wname + " for pane " + buf;
    proc_exception(ex, msg);
  }
}
...
void Roccom_base::new_attribute(const std::string &wa, const char loc,
                                const int type, int size,
...
// Register a new attribute with given name
void Roccom_base::delete_attribute(const std::string &wa) {
  try {
    if (_debug) std::cerr << "Roccom: delete attribute \"" << wa << std::endl;
...
void Roccom_base::set_size(const std::string &wa, int pid, int nitems, int ng) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Set size for attribute \"" << wa << '"'
                << " on pane " << pid << " to " << nitems << " items with "
                << ng << " ghosts" << std::endl;
    }
...
class COM_Object_derived : public COM_Object {
 public:
  static bool cast_obj() {
    ...
    int                ierr;
    (reinterpret_cast<COM_Object *>((void *)&obj)->*f)(&ierr);
 ...
 protected:
  ...
  void validate_object(int *ierr) {
    if (_cookie != DERIVED_COOKIE)
      *ierr = -1;
    else
      *ierr = COM_Object::validate_object();
  }
...
void Roccom_base::set_object(const std::string &wa, const int pid,
  ...
  if (_cppobj_casting == -1) _cppobj_casting = COM_Object_derived::cast_obj();
...
// Associates an object with a specific window.
void Roccom_base::get_object(const std::string &wa, const int pid, void **ptr) {
  int strd;
...
void Roccom_base::set_array(const std::string &wa, const int pid, void *addr,
                            int strd, int cap, bool is_const) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Set array for \"" << wa << "\" on pane " << pid
                << " to " << addr << " with stride ";
      if (strd)
        std::cerr << strd;
      else
...
void Roccom_base::allocate_array(const std::string &wa, const int pid,
  ...
  try {
    if (_debug) {
      std::cerr << "Roccom: Allocate array for \"" << wa << "\" on pane " << pid
                << " with stride ";
      if (strd)
        std::cerr << strd;
      else
...
void Roccom_base::resize_array(const std::string &wa, const int pid,
  ...
  try {
    if (_debug) {
      std::cerr << "Roccom: Resize array for \"" << wa << "\" on pane " << pid
                << " with stride ";
      if (strd < 0)
        std::cerr << "==current";
      else if (strd)
...
void Roccom_base::append_array(const std::string &wa, const int pid,
  ...
  try {
    ...
    std::string s("When processing attribute ");
    s.append(wa);
    proc_exception(ex, s);
  }
}

void Roccom_base::use_attribute(const std::string &waname,
                                const std::string &pwaname, int withghost,
                                const char *cndname, int val) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Using attribute \"" << pwaname << "\" onto \""
                << waname << '"' << std::endl;
    }
    ...
    if (cndname && *cndname != 0) split_name(cndname, cawname, caaname);

    Attribute *cnd =
        caaname.empty() ? NULL : get_window(cawname).attribute(caaname);
    if (!caaname.empty() && cnd == NULL)
      throw COM_exception(COM_ERR_ATTRIBUTE_NOTEXIST, cndname);

    get_window(awname).inherit(get_window(pawname).attribute(paaname), aaname,
                               Pane::INHERIT_USE, withghost, cnd, val);
    _errorcode = 0;
  } catch (COM_exception ex) {
...
void Roccom_base::clone_attribute(const std::string &waname,
                                  const std::string &pwaname, int withghost,
                                  const char *cndname, int val) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Cloning attribute \"" << pwaname << "\" onto \""
                << waname << '"' << std::endl;
    }
    std::string pawname, paaname;
    split_name(pwaname, pawname, paaname);
    ...
    get_window(awname).inherit(get_window(pawname).attribute(paaname), aaname,
                               Pane::INHERIT_CLONE, withghost, cnd, val);
    _errorcode = 0;
  } catch (COM_exception ex) {
...
void Roccom_base::copy_attribute(const std::string &waname,
                                 const std::string &pwaname, int withghost,
                                 const char *cndname, int val) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Copying attribute \"" << pwaname << "\" onto \""
                << waname << '"' << std::endl;
    }
    std::string pawname, paaname;
    split_name(pwaname, pawname, paaname);
    ...
    get_window(awname).inherit(get_window(pawname).attribute(paaname), aaname,
                               Pane::INHERIT_COPY, withghost, cnd, val);
    _errorcode = 0;
  } catch (COM_exception ex) {
...
void Roccom_base::copy_attribute(int trg_hdl, int src_hdl, int withghost,
                                 int ptn_hdl, int val) {
  try {
    if (_debug) {
    ...
    if (_attr_map.is_immutable(trg_hdl)) throw COM_exception(COM_ERR_IMMUTABLE);
    ...
    sout << "When copying attribute " << src_hdl << " onto " << trg_hdl
         << " with pattern attribute " << ptn_hdl;
    proc_exception(ex, sout.str());
  }
}

void Roccom_base::deallocate_array(const std::string &wa, const int pid) {
  try {
    if (_debug)
...
// Register a new attribute with given name
void Roccom_base::get_attribute(const std::string &wa, char *loc, int *type,
...
void Roccom_base::get_size(const std::string &wa, int pid, int *nitems,
                           int *ng) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Get size for attribute \"" << wa << '"'
...
void Roccom_base::get_array(const std::string &wa, const int pid,
                            Pointer_descriptor &addr, int *strd, int *cap,
                            bool is_const) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Get array for attribute \"" << wa << '"'
...
int Roccom_base::get_status(const std::string &wa, int pid) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Get status for attribute \"" << wa << '"'
                << " on pane " << pid << std::endl;
    }
...
void Roccom_base::copy_array(const std::string &wa, const int pid, void *val,
                             int v_strd, int v_size, int offset) {
  try {
    if (_debug) {
      std::cerr << "Roccom: Copy array for attribute \"" << wa << '"'
                << " on pane " << pid << std::endl;
    }
...
int Roccom_base::check_bounds(const std::string &wa, int pane_id) { return 0; }
...
void Roccom_base::set_f90pointer(const std::string &waname, void *ptr,
                                 Func_ptr f, long int len) {
  try {
    if (_debug)
      std::cerr << "Roccom: set pointer \"" << waname << " to " << ptr
                << std::endl;
    ...
    if (a->data_type() == COM_F90POINTER) {
      ...
      if (_f90ptr_treat == FPTR_INSERT || _f90ptr_treat == FPTR_APPEND) {
        ...
        *(void **)((char *)a->pointer() + offs.first) = NULL;
        (*(Func2)f)(ptr, (char *)a->pointer() + offs.first, len,
                    *(long int *)((char *)a->pointer() + offs.second));
      } else {
...
void Roccom_base::get_f90pointer(const std::string &waname, void *ptr,
                                 Func_ptr f, long int len) {
  try {
    if (_debug)
      std::cerr << "Roccom: get pointer \"" << waname << " into " << ptr
                << std::endl;
...
MPI_Comm Roccom_base::get_communicator(const std::string &wname) {
  try {
    if (_debug)
      std::cerr << "Roccom: get the communicator of window \"" << wname
                << std::endl;
    if (COMMPI_Initialized()) {
...
void Roccom_base::get_panes(const std::string &wname,
                            std::vector<int> &paneids_vec, int rank,
                            int **pane_ids) {
  try {
    if (_debug) {
      std::cerr << "Roccom: get pane ids of window \"" << wname << "\" on ";
      if (rank == -2)
        std::cerr << " this process";
      else if (rank == -1)
...
void Roccom_base::get_windows(std::vector<std::string> &names) {
  try {
    if (_debug) {
...
void Roccom_base::get_modules(std::vector<std::string> &names) {
  try {
    if (_debug) {
...
void Roccom_base::get_attributes(const std::string &wname, int *natts,
  ...
  try {
    if (_debug)
      std::cerr << "Roccom: get attributes of window \"" << wname << '"'
                << std::endl;
...
void Roccom_base::get_connectivities(const std::string &wname, int pane_id,
                                     int *natts, std::string &str,
                                     char **names) {
  try {
    if (_debug)
...
void Roccom_base::get_parent(const std::string &waname, int pane_id,
  ...
  try {
    ...
    if (_debug) std::cerr << "Roccom: Got parent name: " << str << std::endl;
...
int Roccom_base::get_window_handle(const std::string &wname) {
  int n(-1);
  try {
    if (_debug)
...
Window *Roccom_base::get_window_object(int hdl) {
  if (hdl <= 0)
...
const Window *Roccom_base::get_window_object(int hdl) const {
  if (hdl <= 0)
    throw COM_exception(COM_ERR_INVALID_WINDOW_HANDLE,
...
int Roccom_base::get_attribute_handle_const(const std::string &waname) {
  int n(-1);
  try {
    if (_debug)
      std::cerr << "Roccom: get const handle of attribute \"" << waname
                << "\": ";
...
int Roccom_base::get_attribute_handle(const std::string &waname) {
  int n(-1);
  try {
    if (_debug)
...
int Roccom_base::get_function_handle(const std::string &wfname) {
  int n(-1);
  try {
    if (_debug)
...
void Roccom_base::set_function(const std::string &wfname, Func_ptr ptr,
  ...
  try {
    if (_debug) {
      int n = intents.size();
      std::cerr << "Roccom: init function \"" << wfname << '"' << " to " << ptr
                << " with " << n << " parameters";
...
template <class T>
void Roccom_base::set_member_function_helper(const std::string &wfname, T ptr,
...
void Roccom_base::set_member_function(const std::string &wfname, Func_ptr ptr,
                                      const std::string &waname,
                                      const std::string &intents,
                                      const COM_Type *types, bool ff) {
  set_member_function_helper(wfname, ptr, waname, intents, types, ff);
}
...
int Roccom_base::get_num_arguments(const std::string &wf) {
  std::string wname, fname;
...
int Roccom_base::get_num_arguments(const int wf) {
...
void Roccom_base::call_function(int wf, int count, void **args, const int *lens,
                                bool from_c) {
  ...
  try {
    ...
    if (verb <= 0)
      ...
      verb = (verb + 1) % 2 + 1;
    if (_debug) {
      std::cerr << "Roccom: CALL(" << _depth << ") " << _func_map.name(wf);
      if (_debug) std::cerr << '(';
    }
    ...
    count += offset;
    args -= offset;
    if (count > func->num_of_args()) throw COM_exception(COM_ERR_TOO_MANY_ARGS);
    ...
    for (int i = offset; i < count; ++i) {
      ...
      if (func->is_literal(i)) {
        ...
        if (type == COM_CHARACTER || type == COM_CHAR || type == COM_STRING) {
          if (type == COM_STRING) {
            ...
            if (plen) {  // Append the length info
              *plen = (char *)NULL + std::strlen((char *)ps[i]);
              ++plen;
              ++lcount;
            }
          } else if (plen) {
          ...
          ps[i] = &strs[i][0] +
                  (sizeof(int) - ((long int)&strs[i][0]) % sizeof(int));
          if (intent == 'i' || intent == 'I' || intent == 'b' || intent == 'B')
        ...
        if (h == 0 && func->intent(i) <= 'Z') {
          ...
          ps[i] = NULL;
          if (_debug) std::cerr << "ZERO ATTRIBUTE HANDLE";
          continue;
        }
        ...
        if (_attr_map.is_immutable(h) && toupper(func->intent(i)) != 'I') {
...
// RAF      if (comm!=MPI_COMM_NULL) MPI_Barrier( comm);
    ...
    msg.append(" with handle ");
    msg.append(buf);
    proc_exception(ex, msg);
  }
}

void Roccom_base::set_function_verbose(int i, int level) {
  _func_map.verbs[i] = level;
}

void Roccom_base::set_profiling(int i) {
  if (_debug) std::cerr << "Roccom: init profiling level to " << i << std::endl;
  _profile_on = i;
...
void Roccom_base::set_profiling_barrier(int hdl, MPI_Comm comm) {
  if (hdl == 0) return;
  try {
    ...
    if (_debug) {
      std::cerr << "Roccom: init profiling barrier for function handle " << hdl
                << std::endl
                << "Roccom:\tFunction: " << _func_map.name(hdl)
...
void Roccom_base::print_profile(const std::string &fname,
...
---------------------\n",
  ...
  for (i = profs.begin(), iend = profs.end(); i != iend; ++i) {
    std::fprintf(of, "%32.32s%12d%14g%14g\n", _func_map.name(i->second).c_str(),
                 _func_map.counts[i->second], _func_map.wtimes_tree[i->second],
...
Window &Roccom_base::get_window(const std::string &wname) {
  Window **w = _window_map.find(wname).second;
  if (w == NULL)
...
Attribute &Roccom_base::get_attribute(const int handle) {
  Attribute *attr = NULL;

  if (handle > 0 && handle < _attr_map.size()) attr = _attr_map[handle];
  if (attr == NULL) {
    static char buf[10];
...
const Attribute &Roccom_base::get_attribute(const int handle) const {
...
Function &Roccom_base::get_function(const int handle) {
  Function *func = NULL;

  if (handle > 0 && handle < _func_map.size()) func = _func_map[handle];
...
const Function &Roccom_base::get_function(const int handle) const {
...
void Roccom_base::proc_exception(const COM_exception &ex,
                                 const std::string   &s) {
  extern void printStackBacktrace();
  _errorcode = ex.ierr;
