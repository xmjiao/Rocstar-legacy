...
const int Connectivity::_sizes[TYPE_MAX_CONN][SIZE_MAX_CONN] = {
    ...
    {ST1, 1, 1, 2, 2, 1, 0},      {ST2, 2, 1, 4, 4, 4, 1},
    {ST3, 3, 1, 8, 8, 12, 6},     {BAR2, 1, 1, 2, 2, 1, 0},
    {BAR3, 1, 2, 3, 2, 1, 0},     {TRI3, 2, 1, 3, 3, 3, 1},
    {TRI6, 2, 2, 6, 3, 3, 1},     {QUAD4, 2, 1, 4, 4, 4, 1},
    {QUAD8, 2, 2, 8, 4, 4, 1},    {QUAD9, 2, 2, 9, 4, 4, 1},
    {TET4, 3, 1, 4, 4, 6, 4},     {TET10, 3, 2, 10, 4, 6, 4},
    {PYRIMID5, 3, 1, 5, 5, 8, 5}, {PYRIMID14, 3, 2, 14, 5, 8, 5},
    {PRISM6, 3, 1, 6, 6, 9, 5},   {PRISM15, 3, 2, 15, 6, 9, 5},
    {PRISM18, 3, 2, 18, 6, 9, 5}, {HEX8, 3, 1, 8, 8, 12, 6},
    {HEX20, 3, 2, 20, 8, 12, 6},  {HEX27, 3, 2, 27, 8, 12, 6}};
...
const int *Connectivity::get_addr(int i, int j) const {
  ...
  if (i < 0 || i >= size_of_items())
    throw COM_exception(COM_ERR_INDEX_OUT_OF_BOUNDS,
                        append_frame(fullname(), Attribute::get_addr));
...
void Connectivity::set_offset(Size offset) { _offset = offset; }
  ...
  if (!is_structured())
    ...
    switch (_size_info[TYPE_ID]) {
      case ST1:
        ...
        break;
      case ST2:
        n = (sizes[0] - 1) * (sizes[1] - 1);
        break;
      case ST3:
        n = (sizes[0] - 1) * (sizes[1] - 1) * (sizes[2] - 1);
        break;
      default:
        COM_assertion(false);  // Should never reach here.
    }
    return std::max(0, n);
  }
}
...
Connectivity::Size Connectivity::size_of_real_elements() const {
  if (!is_structured())
    ...
    switch (_size_info[TYPE_ID]) {
      case ST1:
        n = sizes[0] - 1 - 2 * _ngitems;
        break;
      case ST2:
        n = (sizes[0] - 1 - 2 * _ngitems) * (sizes[1] - 1 - 2 * _ngitems);
        break;
      case ST3:
        n = (sizes[0] - 1 - 2 * _ngitems) * (sizes[1] - 1 - 2 * _ngitems) *
            (sizes[2] - 1 - 2 * _ngitems);
...
Connectivity::Size Connectivity::size_of_ghost_nodes() const {
  if (!is_structured())
    ...
    switch (_size_info[TYPE_ID]) {
      ...
      case ST3:
        return sizes[0] * sizes[1] * sizes[2] - (sizes[0] - 2 * _ngitems) *
                                                    (sizes[1] - 2 * _ngitems) *
                                                    (sizes[2] - 2 * _ngitems);
      default:
        COM_assertion(false);  // Should never reach here.
...
Connectivity::Size Connectivity::size_of_real_nodes() const {
  if (!is_structured())
    ...
    switch (_size_info[TYPE_ID]) {
      case ST1:
        return sizes[0] - 2 * _ngitems;
      case ST2:
        return (sizes[0] - 2 * _ngitems) * (sizes[1] - 2 * _ngitems);
      case ST3:
        return (sizes[0] - 2 * _ngitems) * (sizes[1] - 2 * _ngitems) *
               (sizes[2] - 2 * _ngitems);
      default:
...
const int *Connectivity::get_size_info(const std::string &aname) {
  ...
  switch (aname[1]) {
    case 'b':
      ...
      if (aname[idx] == '2') {
        COM_assertion(_sizes[BAR2][TYPE_ID] == BAR2);
        return _sizes[BAR2];
      }
      if (aname[idx] == '3') {
        COM_assertion(_sizes[BAR3][TYPE_ID] == BAR3);
        return _sizes[BAR3];
      }
      break;
    case 't':
      while (aname[idx] == '0') idx++;
      if (aname[idx] == '3') {
        COM_assertion(_sizes[TRI3][TYPE_ID] == TRI3);
        return _sizes[TRI3];
      }
      if (aname[idx] == '6') {
        COM_assertion(_sizes[TRI6][TYPE_ID] == TRI6);
        return _sizes[TRI6];
      }
      break;
    case 'q':
      while (aname[idx] == '0') idx++;
      if (aname[idx] == '4') {
        COM_assertion(_sizes[QUAD4][TYPE_ID] == QUAD4);
        return _sizes[QUAD4];
      }
      if (aname[idx] == '8') {
        COM_assertion(_sizes[QUAD8][TYPE_ID] == QUAD8);
        return _sizes[QUAD8];
      }
      if (aname[idx] == '9') {
        COM_assertion(_sizes[QUAD9][TYPE_ID] == QUAD9);
        return _sizes[QUAD9];
      }
      break;
    case 'T':
      while (aname[idx] == '0') idx++;
      if (aname[idx] == '4') {
        COM_assertion(_sizes[TET4][TYPE_ID] == TET4);
        return _sizes[TET4];
      }
      if (aname[idx] == '1' && aname[idx + 1] == '0') {
        COM_assertion(_sizes[TET10][TYPE_ID] == TET10);
        return _sizes[TET10];
      }
      break;
    case 'H':
    case 'B':
      while (aname[idx] == '0') idx++;
      if (aname[idx] == '8') {
        COM_assertion(_sizes[HEX8][TYPE_ID] == HEX8);
        return _sizes[HEX8];
      }
      if (aname[idx] == '2') {
        if (aname[idx + 1] == '0') {
          COM_assertion(_sizes[HEX20][TYPE_ID] == HEX20);
          return _sizes[HEX20];
        }
        if (aname[idx + 1] == '7') {
          COM_assertion(_sizes[HEX27][TYPE_ID] == HEX27);
          return _sizes[HEX27];
        }
      }
      break;
    ...
    case 's':
      if (aname[idx++] == 't') {
        while (aname[idx] == '0') idx++;
        if (aname[idx] == '3') {
          COM_assertion(_sizes[ST3][TYPE_ID] == ST3);
          return _sizes[ST3];
        }

        if (aname[idx] == '2') {
          COM_assertion(_sizes[ST2][TYPE_ID] == ST2);
          return _sizes[ST2];
        }

        if (aname[idx] == '1') {
          COM_assertion(_sizes[ST1][TYPE_ID] == ST1);
          return _sizes[ST1];
        }
      }
      break;
