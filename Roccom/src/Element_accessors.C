...
Element_node_enumerator::Element_node_enumerator(const Pane *pane, int i,
                                                 const Connectivity *conn)
    : _pane(pane),
      _conn(_pane->is_structured()
                ? NULL
                : (conn == NULL ? pane->connectivity(i) : conn)) {
  if (_pane->is_unstructured()) {
    if (conn == NULL && _conn != NULL)
      i -= _conn->index_offset() + 1;
    else
      --i;
...
int Element_node_enumerator::vertex(int lvid, bool level) const {
...
Facet_node_enumerator::Facet_node_enumerator(const Element_node_enumerator *ene,
                                             int                            k)
    : _ene(ene), _k(k), _ne(0) {
  if (ene->dimension() == 2) {
    ...
    switch (ene->size_of_nodes()) {
      ...
      default:
        COM_assertion_msg(false, "Not supported yet.");
    }
  }
}
...
int Facet_node_enumerator::operator[](int i) const {
  if (_ne > 1)
    ...
    if (i < 2)
      return (*_ene)[(_k + i) % _ene->size_of_edges()];
    else
      return (*_ene)[_ene->size_of_edges() + _k];
  }
}

void Facet_node_enumerator::get_nodes(std::vector<int> &nodes, bool quad_ret) {
